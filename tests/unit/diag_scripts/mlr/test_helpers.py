...
def test_load_mlr_models(mock_dirname, mock_walk, mock_importlib):
    ...
    modules = [
        f"esmvaltool.diag_scripts.mlr.models.{mod}"
        for mod in [
            "test",
            "__init__",
            "root2.__init__",
...
DF_3_OUT = pd.DataFrame(
    {"c": [1.0, 3.0, 5.0], "d": [-2.0, 0.0, 2.0]}, index=[0, 2, 4]
)
...
TEST_SQUARE_ROOT_METADATA = [
    (
        ...
        iris.cube.Cube(0, **METADATA_OUT[idx]._asdict()),
    )
    for idx in range(len(METADATA_IN))
]
        ...
        if isinstance(output, type):
            with pytest.raises(output):
                mlr.datasets_have_mlr_attributes(
                    datasets, log_level=log_level, mode=mode
                )
            return
        out = mlr.datasets_have_mlr_attributes(
            datasets, log_level=log_level, mode=mode
        )
        if output == 0:
            assert out is True
...
@pytest.mark.parametrize(
    "cfg,ancestors,check_mlr_attrs,ignore,output,n_logger", TEST_GET_INPUT_DATA
)
@mock.patch("esmvaltool.diag_scripts.mlr.io.netcdf_to_metadata", autospec=True)
@mock.patch("esmvaltool.diag_scripts.mlr.logger", autospec=True)
def test_get_input_data(
    mock_logger,
    mock_netcdf_to_metadata,
    cfg,
    ancestors,
    check_mlr_attrs,
    ignore,
    ...
    if isinstance(output, type):
        with pytest.raises(output):
            mlr.get_input_data(
                cfg, check_mlr_attributes=check_mlr_attrs, ignore=ignore
            )
...
LAT_COORD_0D = iris.coords.AuxCoord(
    0, bounds=[-10, 10], standard_name="latitude", units="degrees"
)
LON_COORD_0D = iris.coords.AuxCoord(
    0, bounds=[-10, 10], standard_name="longitude", units="degrees"
)
LAT_COORD_1D = iris.coords.DimCoord(
    [-10, 50, 60],
    bounds=[[-20, 0], [45, 55], [55, 65]],
    standard_name="latitude",
    units="degrees",
)
LAT_COORD_1D_2 = iris.coords.DimCoord(
    [70, 220],
    bounds=[[60, 70], [215, 225]],
    standard_name="latitude",
    units="degrees",
)
LON_COORD_1D = iris.coords.DimCoord(
    [70, 220],
    bounds=[[60, 70], [215, 225]],
    standard_name="longitude",
    units="degrees",
)
LAT_COORD_2D = iris.coords.AuxCoord(
    [[0, 2]],
    bounds=[[[-1, 1], [1, 3]]],
    standard_name="latitude",
    units="degrees",
...
CUBE_0_1 = iris.cube.Cube(
    np.arange(2),
    dim_coords_and_dims=[(LON_COORD_1D, 0)],
    aux_coords_and_dims=[(LAT_COORD_0D, ())],
)
CUBE_1_0 = iris.cube.Cube(
    np.arange(3),
    dim_coords_and_dims=[(LAT_COORD_1D, 0)],
...
TEST_LANDSEA_FRACTION_WEIGHTING = [
    ...
    (
        ...
        "land",
        False,
        [[0.0, 6.55200655e-04], [1.0, 0.0], [1.0, 5.66883117e-01]],
    ),
    (
        CUBE_1_1_1,
        "land",
    ...
    (
        ...
        "sea",
        False,
        [[1.0, 0.9993448], [0.0, 1.0], [0.0, 0.43311688]],
    ),
    (
        CUBE_1_1_1,
        "sea",
    ...
    (
        ...
        "sea",
        True,
        [[0.2913361, 0.2911452164], [0.0, 0.2913361], [0.0, 0.1261825836]],
    ),
    (
        CUBE_1_1_1,
        "sea",
...
@pytest.mark.parametrize(
    "cube,area_type,normalize,output", TEST_LANDSEA_FRACTION_WEIGHTING
)
def test_landsea_fraction_weighting(cube, area_type, normalize, output):
    ...
    if isinstance(output, type):
        with pytest.raises(output):
            mlr.get_landsea_fraction_weights(
                cube, area_type, normalize=normalize
            )
        return
    weights = mlr.get_landsea_fraction_weights(
        cube, area_type, normalize=normalize
    )
