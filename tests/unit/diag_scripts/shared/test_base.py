...
def test_group_metadata():
    ...
    assert result == {
        "ta": [
            {"short_name": "ta", "filename": "test_ta.nc"},
        ],
        "pr": [
            {"short_name": "pr", "filename": "test_pr.nc"},
        ],
    }
...
def test_group_metadata_sorted():
    ...
    assert result == {
        "ta": [
            {"short_name": "ta", "dataset": "dataset1"},
            {"short_name": "ta", "dataset": "dataset2"},
        ],
    }
...
def test_rerun_diagnostic_flag(tmp_path, monkeypatch, flag):
    ...
    with shared.run_diagnostic():
        assert not (
            Path(settings["run_dir"]) / "diagnostic_provenance.yml"
        ).exists()
        for file in (
