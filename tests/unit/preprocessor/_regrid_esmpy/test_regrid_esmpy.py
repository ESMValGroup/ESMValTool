...
class TestHelpers(tests.Test):
    def setUp(self):
        ...
        lat_1d_bounds = np.stack([lat_1d_pre_bounds[:-1],
                                  lat_1d_pre_bounds[1:]],
                                 axis=1)
        lat_1d_points = lat_1d_bounds.mean(axis=1)
        lon_1d_pre_bounds = np.linspace(0, 360, 5)
        lon_1d_bounds = np.stack([lon_1d_pre_bounds[:-1],
                                  lon_1d_pre_bounds[1:]],
                                 axis=1)
        ...
        lon_2d_points, lat_2d_points = np.meshgrid(lon_1d_points,
                                                   lat_1d_points)
        (lon_2d_pre_bounds,
         lat_2d_pre_bounds) = np.meshgrid(lon_1d_pre_bounds,
                                          lat_1d_pre_bounds)
        lat_2d_bounds = np.stack([lat_2d_pre_bounds[:-1, :-1],
                                  lat_2d_pre_bounds[:-1, 1:],
                                  lat_2d_pre_bounds[1:, 1:],
                                  lat_2d_pre_bounds[1:, :-1]],
                                 axis=2)
        lon_2d_bounds = np.stack([lon_2d_pre_bounds[:-1, :-1],
                                  lon_2d_pre_bounds[:-1, 1:],
                                  lon_2d_pre_bounds[1:, 1:],
                                  lon_2d_pre_bounds[1:, :-1]],
                                 axis=2)
        ...
        depth_bounds = np.stack([depth_pre_bounds[:-1],
                                 depth_pre_bounds[1:]],
                                axis=1)
        ...
        self.expected_esmpy_lon_corners = np.array(
            [[0., 0., 0., 0., 0.],
             ...
             [270., 270., 270., 270., 270.],
             [360., 360., 360., 360., 360.]])
        self.coords = {'latitude': self.lat_2d,
    ...
    def test_coords_iris_to_esmpy_1d_circular(self):
        (esmpy_lat,
         esmpy_lon,
         esmpy_lat_corners,
         esmpy_lon_corners) = coords_iris_to_esmpy(self.lat_1d,
                                                   self.lon_1d,
                                                   True)
    ...
    def test_coords_iris_to_esmpy_2d_circular(self):
        (esmpy_lat,
         esmpy_lon,
         esmpy_lat_corners,
         esmpy_lon_corners) = coords_iris_to_esmpy(self.lat_2d,
                                                   self.lon_2d,
                                                   True)
        self.assertArrayEqual(esmpy_lat, self.expected_esmpy_lat)
        self.assertArrayEqual(esmpy_lon, self.expected_esmpy_lon)
    ...
    def test_get_grid_circular(self):
        ...
        with mock.patch('ESMF.Grid', MockGrid) as mg:
            ...
            get_grid(self.expected_esmpy_lat,
                     self.expected_esmpy_lon,
                     self.expected_esmpy_lat_corners[:-1],
                     self.expected_esmpy_lon_corners[:-1],
                     True)
    ...
    def test_is_lon_circular_invalid_dimension(self):
        self.assertRaises(NotImplementedError,
                          is_lon_circular, self.lon_3d)

    def test_is_lon_circular_invalid_argument(self):
        self.assertRaises(ValueError,
                          is_lon_circular, None)
    ...
    def test_cube_to_empty_field(self, mock_field):
        ...
        cargs, ckwargs = mock_field.call_args
    ...
    def test_get_grid_representants_3d_src(self,
                                           mock_cube, mock_get_empty_data):
        src = self.cube_3d
        mock_get_empty_data.return_value = mock.sentinel.empty_data
        src_rep, dst_rep = get_grid_representants(src, self.cube)
    ...
    @mock.patch('esmvaltool.preprocessor._regrid_esmpy.get_empty_data')
    @mock.patch('iris.cube.Cube')
    def test_get_grid_representants_2d_src(self,
                                           mock_cube, mock_get_empty_data):
        src = self.cube
        mock_get_empty_data.return_value = mock.sentinel.empty_data
        src_rep, dst_rep = get_grid_representants(src, self.cube)
