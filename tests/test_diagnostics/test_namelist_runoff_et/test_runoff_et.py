...
class RunoffTest(ESMValToolTest):
    """
    Define class for Test metric here
    """
    def __init__(self, **kwargs):
        xmlfile = os.path.dirname(os.path.realpath(__file__)) + os.sep + "namelist_runoff_et.xml"  # the realpath ensures that the file is also foudn wehn nosetests runs from an upper directory
        super(RunoffTest, self).__init__(nml=xmlfile, **kwargs)
    ...
    def get_field_definitions(self):
        ...
        r.update({'mrro' : {'method' : 'uniform', 'filename' : rpath + os.sep + '@{VAR_FILE}', 'ndim' : 2}})
...
class TestDiagnostic(ESMValTestDiagnostic):
    ...
    def test_general_output(self):
        ...
        T = RunoffTest(files=reffiles, subdirectory = 'catchment_analysis_val')  # provide subdirectory within plot_dir
        T.run_nml()
