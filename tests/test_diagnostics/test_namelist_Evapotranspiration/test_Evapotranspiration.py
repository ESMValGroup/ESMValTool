...
class ETTest(ESMValToolTest):
    """
    Define class for Test metric here
    """
    def __init__(self, **kwargs):
        xmlfile = os.path.dirname(os.path.realpath(__file__)) + os.sep + "namelist_Evapotranspiration.xml"  # the realpath ensures that the file is also foudn wehn nosetests runs from an upper directory
        super(ETTest, self).__init__(nml=xmlfile, **kwargs)
    ...
    def get_field_definitions(self):
        ...
        r.update({'et' : {'method' : 'uniform', 'filename' : rpath + os.sep + '@{VAR_FILE}', 'ndim' : 2}})
        r.update({'hfls' : {'method' : 'uniform', 'filename' : rpath + os.sep + '@{VAR_FILE}', 'ndim' : 2}})
        return r
