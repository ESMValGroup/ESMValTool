...
with open(
    os.path.join(os.path.dirname(__file__), "configs", "test_general.yml")
) as file_:
...
class TestMLRModel:
    ...
    def test_create(
        self, mock_mlr_model_init, mock_load_mlr_models, mock_logger
    ):
        ...
        MLRModel.create("test_model", *self.args, **self.kwargs)
        mock_load_mlr_models.assert_called()
        mock_mlr_model_init.assert_called_with(
            mock.ANY, *self.args, **self.kwargs
        )
