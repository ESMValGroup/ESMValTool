...
are available at https://maps.elie.ucl.ac.be/CCI/viewer/ upon registration. Detailed instructions for the
installation and use of the CCI-LC user tools is available on the same page.

Note, that all experiments will be regridded onto the grid of the land cover observations, thus it is
recommended to convert to the coarses resolution which is sufficient for the planned study. For the script
development, ESA-CCI data on 0.5 degree resolution was used.
