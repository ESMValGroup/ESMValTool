...
#main_ESMVALTool="/media/bmueller/Work/GIT/ESMValTool_v1.0"
#main_ESMVALTool="/media/bmueller/Work/GIT/Tagging"
main_ESMValTool="../"
version="1.0 (alpha)"
...
description = """Reporting service for the ESMValTool - Earth System Model Evaluation Tool."""
...
#if all(el in project_info.keys() for el in ['FOLDERS','TAGS']):
#    case="post"
#elif all(el in project_info.keys() for el in ['GLOBAL','DIAGNOSTICS']):
#    case="pre"
#else:
#    assert False, "Project keys not supported!"
...
try:
    ...
    ESMValTool_log = subprocess.check_output("cd " + main_ESMValTool + " && python "+ "main.py "+ project_info['RUNTIME']['xml'], shell=True)
    case="pre"
except Exception, e:
...
if case == "pre":
    ...
    # Home
    left_box='<p>This is the output from the ESMValTool run. Please check if any errors occur!</p>'
    D.home_html(left_box,middle_box=ESMValTool_log,right_box=[project_info['RUNTIME']['xml_name']]) #TODO Errors optional   
    ...
    for currDiag in project_info['DIAGNOSTICS']:
        ...
        requested_vars = list(diag_o.__dict__['var'] for diag_o in requested_vars)
    ...
    #    print(requested_vars)
        ...
        # Diagnostic
        D.diag_html(DKEY,project_info['GLOBAL']['plot_dir'],cfg,[DKEY],full_host,time=(timestamp-datetime.datetime.utcfromtimestamp(0)).total_seconds())
    ...
    # Index
    D.index_html(list_of_vars=list(set(var_list)),list_of_keys=Key_list,version=version,list_of_authors=authors)

elif case == "post":
    
    # Home
    home_text="THIS IS A TAG-NAMELIST GENERATED REPORT! THERE IS NO DIRECT LOG FROM THE ESMVaLTool!"
    title_alt='ESMValTool'
    ESMValTool_Image='<figure><a title=' + title_alt + '><img style="background-color:white; margin-left:auto; margin-right:auto; display:block;" alt=' + title_alt + ' src="'+ '../static/images/ESMValTool-logo.png' +'" width="2000"' +'></a></figure>'
    D.home_html(left_box=home_text,middle_box=ESMValTool_Image,right_box=[project_info['RUNTIME']['xml_name']])  
    ...
    for t in project_info['TAGS'].tags:
        ...
        # Tag-sites
        D.tag_html(DKEY,project_info['FOLDERS'].folders,t.split(","),full_host)
    ...
    # Index
    D.index_html(list_of_vars=project_info['TAGS'].tags,list_of_keys=Key_list,version=version,list_of_authors=authors)
