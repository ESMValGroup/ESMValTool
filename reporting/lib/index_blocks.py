...
#    input_desc=[inputs[0] if len(inputs)==1 else ", ".join(inputs)][0]
    Headline='Results from the ESMValTool'#'Diagnostics results for ' + input_desc
    lines=[
        '<!DOCTYPE html>',
        '<html>',
        '<head>',
            ...
            '<link rel="stylesheet" type="text/css" href="{{url_for("static",filename="css/styles.css")}}" media="print" />',
        '</head>',
        '<body>',
        '<div id="header">',
	    '<h1>' + Headline + '</h1>',
...
def index_tabs(inputs):
    ...
    def tabline(hook,info=None):
        """ returns ref for tag """
        if info is None:
            res=['<li><a href="{{url_for("' + hook + '")}}">' + hook.upper() + '</a></li>']
        elif isinstance(info,list):
            res=[]
            for i in info:
                if isinstance(i['name'],(str,unicode)):
                    res.extend(['<li><a href="{{url_for("' + hook + '",tagname="' + i['key'] + '")}}">' + i['name'].upper() + '</a></li>'])
                elif isinstance(i['name'],list):
                    res.extend(['<li><a href="{{url_for("' + hook + '",tagname="' + i['key'] + '")}}">' + "/".join(i['name']).upper() + '</a></li>'])
                else:
...
def index_foot(version,inputs):
    """ this block gives the head of the html index """
    inputs=['<a href="mailto:'+ string +'">'+ string +'</a>' if re.search('@',string) else string for string in inputs] 
    input_desc=[inputs[0] if len(inputs)==1 else ", ".join(inputs)][0]
    lines=[
            '<form>',
