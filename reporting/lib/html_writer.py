...
class HTML_writer(object):
    ...
    def _get_img_files(self,directory):
        ...
        for pat in ['png','jpg','jpeg','tiff']: #any other formats?
            ...
            L_loc , N = self._get_files_in_directory(directory,'*.'+pat,False)   
    ...
    def make_PDF_list(self,folder,name,restrictor,host):
        ...
        file_list=['<object data=".'+f+'" type="application/pdf"></object>' for f in file_list]
    ...
    def make_IMG_list(self,folder,name,restrictor,host,time=None):
        ...
        file_list=['<figure><a href=' + "http://" + host + "/" + f + ' title=' + MD.read(f).get_dict()['ESMValTool']['block'][0] + '><img src=".'+f+'"></a><figcaption>'+ MD.read(f).get_dict()['ESMValTool']['caption'][0] + ' (V. ' +  MD.read(f).get_dict()['ESMValTool']['built'][0] + ')' +'</figcaption></figure>' for f in file_list]

        file_list = [item for sublist in map(None, file_list, breaks) for item in sublist][:-1]
    ...
    def correct_file_list(self,file_list,name,restrictor_or_time):
        ...
        file_list.sort(key=lambda x: MD.read(x).get_dict()['ESMValTool']['block'][0])
        
        if isinstance(restrictor_or_time,(str,unicode)):
            actual_file_list=[(restrictor_or_time in MD.read(x).get_dict()['ESMValTool']['tags']) for x in file_list]
        elif isinstance(restrictor_or_time,list):  
            actual_file_list=[all(r in MD.read(x).get_dict()['ESMValTool']['tags'] for r in restrictor_or_time) for x in file_list]
        elif isinstance(restrictor_or_time,float):
            actual_file_list=[MD.read(x).get_dict()['ESMValTool']['built'][0]>restrictor_or_time for x in file_list]#TODO implement switch
        else:
...
#            assert False, "No Files found with restrictor '" + str((", ").join(restrictor_or_time) if isinstance(restrictor_or_time,list) else restrictor_or_time) + "'!"
    ...
    def diag_html(self,name="tab",folder="./",add_cfg="config",restrictor=[""],host="0.0.0.0:5000",time=None):
        ...
        IMG_file_list = self.make_IMG_list(folder,name,restrictor,host,time)
        PDF_file_list = []
...
#        <embed src="http://example.com/the.pdf" width="500" height="375" type='application/pdf'>
    ...
    def index_html(self,list_of_vars=["var1","var2"],list_of_keys=["key1","key2"],version="0.0alpha",list_of_authors=["person1","person2"]):
        ...
        Tabs=index_blocks.index_tabs(list_of_keys)
