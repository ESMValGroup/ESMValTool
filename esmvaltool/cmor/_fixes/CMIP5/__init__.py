...
def add_irregular_bounds(cube):
    """Add bounds (vertices) for irregular grids.
    ...
    rlat_to_idx = InterpolatedUnivariateSpline(rlat, np.arange(len(rlat)), k=1)
    rlon_to_idx = InterpolatedUnivariateSpline(rlon, np.arange(len(rlon)), k=1)
    ...
    for (i, j) in [(0, 0), (0, 1), (1, 1), (1, 0)]:
        ...
        lon_vertices.append(
            map_coordinates(
                cube.coord('longitude').points, [rlat_v, rlon_v], mode='wrap'))
