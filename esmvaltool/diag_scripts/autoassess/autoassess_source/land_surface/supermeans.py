...
seasons are averaged periodically. Annual 'Supermeans' are averages over several
full years.
"""
...
class NoClimateYearError(ValueError):
    pass
...
def get_supermean(name, season, data_dir):
    ...
    :param name: Cube name. Should be CF-standard name. If no CF-standard name
                 ...
                 is used as name.
    :param season: Supermean for a season (including annual). ['ann', 'djf', 'mam', 'jja', 'son']
    :param data_dir: Directory containing pickled cubes of model output data for
                     supermeans.
    ...
    Supermeans are only applied to full climatological years (Starting December 1st).
    """
    name_constraint = iris.Constraint(name=name)

    cubes_path = os.path.join(data_dir, 'cubes.pickle')
    with open(cubes_path, 'r') as fh:
    ...
    if season in ['djf', 'mam', 'jja', 'son']:
        ...
        raise ValueError("Argument 'season' must be one of ['ann', 'djf', 'mam', 'jja', 'son']. "
                         "It is: " + str(season))
...
def contains_full_climate_years(cube):
    ...
    A climate year begins at YYYY-12-01 00:00:00, and ends at YYYY-12-01 00:00:00.
    ...
    :param Cube: Cube.
    :returns: True if first and last time bound in cube are at YYYY-12-01 00:00:00.
    :rtype: boolean
    """
    ...
    if is_24h_sampled(cube):
        ...
        return all(start == end and start in year_boundaries and end in year_boundaries
...
def is_24h_sampled(cube):
    ...
    for cm in cube.cell_methods:
        if cm.method == 'mean' and 'time' in cm.coord_names:
            meaning_periods.extend(cm.intervals)
...
def periodic_mean(cube, period=None):
    ...
    Note: In the returned cube, the bounds for each period are the start boundary
    of the first period that is averaged over, and the end boundary of the last
    period that is averaged over.
    """
    if not period in [None, 'month', 'season']:
        raise InvalidPeriod('Invalid period: ' + str(period))


    if not contains_full_climate_years(cube):
        raise NoClimateYearError("Cube does not contain climate year(s).")
    ...
    if period == 'month':
        ...
        all_months = set(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul',
                          'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
...
def _add_categorised_coord(cube, name, from_coord, category_function,
                           units='1'):
    """
...
def time_average_by(cube, periods='time'):
    """Average cube over time or over periods, i. e. time-based categorical
    ...
    idx_obj[cube.coord_dims('time')[0]] = slice(None)  # [None, slice(None), None] == [np.newaxis, :, np.newaxis]
    ...
    if periods == ['time']:  # duration weighted averaging
        ...
        durations_cube = durations_cube.aggregated_by(periods, iris.analysis.SUM)
    ...
    time_averaging_method = iris.coords.CellMethod(method='mean', coords=periods)
...
def durations(time_coord):
    ...
    durations = np.array([bounds[1] - bounds[0] for bounds in time_coord.bounds])
    return durations
