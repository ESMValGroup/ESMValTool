...
import iris
from iris.analysis.calculus import curl as curl3d
...
def strmfn(uwnd, vwnd):
    '''
    Function to return the streamfunction of u and v winds
    '''

    # Remove the mask from the data (replace with zeros)
    if 'mask' in dir(uwnd.data):
        uwnd.data = np.ma.filled(uwnd.data, 0)
    if 'mask' in dir(vwnd.data):
        vwnd.data = np.ma.filled(vwnd.data, 0)
...
def velpot(uwnd, vwnd):
    '''
    Function to return the velocity potential of u and v winds
    '''
    ...
    # Create a VectorWind instance to handle the computation of velocity potental
    w = VectorWind(uwnd, vwnd)
...
def divergence(uwnd, vwnd):
    '''
    Function to return the divergence of u and v winds
    '''
    ...
    # Create a VectorWind instance to handle the computation of divergence
    w = VectorWind(uwnd, vwnd)
...
def curl(uwnd, vwnd):
    '''
    Function to return the curl of u and v winds (or wind stresses)
    '''

    # Set the units. By default we assume m s-1 (wind speed)
    thisunit = Unit('m s-1')
    # but we have to change it for wind stresses where Pa = N m-2
    if 'stress' in uwnd.name():
        thisunit = Unit('Pa')

    # Change the standard names to be something acceptable to the curl routine
    uwnd.standard_name = 'x_wind'
    vwnd.standard_name = 'y_wind'

    # Provide sensible units
    uwnd.units = thisunit
    vwnd.units = thisunit

    # Remove problematic auxillary coordinate
    vm.remove_aux_factory(uwnd, 'altitude')
    vm.remove_aux_factory(vwnd, 'altitude')

    # Remove coordinate 'model_level_number' if there is one
    vm.remove_scalar_coord(uwnd, 'model_level_number')
    vm.remove_scalar_coord(vwnd, 'model_level_number')

    # Add a scalar coordinate if we need one
    vm.add_scalar_coord(uwnd, 'level_height')
    vm.add_scalar_coord(vwnd, 'level_height')

    # Run the curl
    curl_result = curl3d(uwnd, vwnd)

    # Remove the units again
    outcube = curl_result[2]
    print 'Curl units are ', outcube.units
    outcube.units = ''
...
def log10(incube):
    ...
    '''
    return iris.analysis.maths.log10(incube)
...
def ocean_only(incube):
    ...
    outcube.data = np.ma.array(incube.data, mask=mask)
    return outcube


# ******** LAND_ONLY *******************

def land_only(incube):
    '''
    Function to mask out the ocean points
    '''
...
def divide(numerator_cube, denominator_cube):
    """Divide the first cube by the second cube but only after all regions where the
    denominator are zero are set to missing data.
    """

    mask = denominator_cube.data == 0.0
    numerator_cube_copy = numerator_cube.copy()
    numerator_cube_copy.data = np.ma.array(numerator_cube.data, mask=mask)
    out_cube = numerator_cube_copy.copy()
    out_cube.data = numerator_cube_copy.data / denominator_cube.data
    return out_cube
