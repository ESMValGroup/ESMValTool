...
def fix_coords(incube):
    ...
    if 'grid_longitude' in incube.coords():
        incube.coord('grid_longitude').rename('longitude')
    if 'grid_latitude' in incube.coords():
        incube.coord('grid_latitude').rename('latitude')
    if u'longitude' in incube.coords():
        incube.coord(u'longitude').rename('longitude')
    if u'latitude' in incube.coords():
        incube.coord(u'latitude').rename('latitude')
...
def regrid_cube(incube, newgrid, scheme_str='AreaWeighted', comp='none'):
    ...
    if incube_str != newgrid_str:
        ...
        if regrided_cube is None:
            ...
            if comp != 'none' and globalvar.store_regrids:
...
def roll_cube(cube, region=None):
    """
    ...
    region = (tuple) optional region that we want data to cover in form
             [(coord_name, (min, max)), (coord_name, (min, max)), ...]
    """
    if region:
        intersect_kwargs = {coord: crange for (coord, crange) in region}
    else:
        intersect_kwargs = {'longitude': (-180, 180), 'latitude': (-90, 90)}
    return cube.intersection(**intersect_kwargs)
...
def area_avg(cube):
    ...
    area_average = np.ma.average(cube.data, weights=cos_2d_array)

    return area_average


def add_colorbar(mappable, units=None, pad=0.15, orientation='horizontal'):
    # adds a colorbar to a plot with some standard keywords set
    # pad = (float) gap between plot and colourbar
    
    cbar_kwargs = {'pad': pad, 'orientation': orientation, 'extend': 'both'}

    cax, cbar_kwargs_filter = matplotlib.colorbar.make_axes(plt.gca(),
                                                            **cbar_kwargs)
    cb = plt.colorbar(mappable, cax=cax,
                      format=ScalarFormatter(useOffset=False),
                      **cbar_kwargs_filter)
...
def file_name(page_num, page_title):
    ...
    Returned:
    thumb_file = optional thumbnail file (only different if publication)
    """
    ...
    if globalvar.pub:
        ...
        thumb_file = page_num+'_'+page_title+'.png'
    else:
...
def extract_no_diurnal(all_data_cube, all_constraints=None):
    ...
    for dim in dims_name_list:
        if 'pseudo_level' in dim:
            pseudo_name_temp = dim
            if this_data_cube.coords(pseudo_name_temp)[0].shape[0] > 1:
                pseudo_name = dim
...
def plot_type_test(incube):
    ...
    'lat_lon_regional' = A contour plot of a region of the globe
    ...
    # If two dimensions then return one of the contour plot names
        ...
        plot_type = 'lat_lon'
        if len(incube.coords(axis='x')) > 0:
            if is_regional(incube):
                plot_type = 'lat_lon_regional'
...
def perform_equation(equation, data_dict, page_title, key, rms_list=None,
    ...
    # If any components are regional set this as the regridded component
    #  and change the regridding scheme
    scheme_str = 'AreaWeighted'
    is_regional_bool = False
    for component in component_set:
        if is_regional(data_dict[component]):
            min_comp = component
            scheme_str = 'Linear'
            print 'Component ' + component + \
                ' is regional. Regridding to this using Linear method'
            is_regional_bool = True

    # If using observations in the equation then regrid to a coarse grid
    # (either N96 or the obs grid if less than N96)
    if using_obs and not is_regional_bool:
        if min_resn < 192:
    ...
    if same_levels:
        # Remove scalar coordinate that interfere with cube comparisons
        for component in component_list:
            for coord in ('model_level_number', 'realization'):
                remove_scalar_coord(data_regrid_dict[component], coord)
        ...
        if multi_levels:
            try:
                ...
                print "ERROR: ", sys.exc_info()
                print 'ERROR: Failed to get cubes onto the same levels, ' + \
                      'possibly due to extra coordinates in one of the ' + \
                      'cubes. Information on cubes attempting to pass into ' + \
                      'intersection_of_cubes follows:'
                for c in range(n_components):
                    print 'component_list[', c, '] = ', component_list[c]
                    print data_regrid_dict[component_list[c]]
                if globalvar.debug:
                    pdb.set_trace()
                else:
                    globalvar.error = True
    ...
    # Remove problematic auxillary coordinate
    remove_aux_factory(toplot_cube, 'altitude')
...
def remove_aux_factory(cube, coord_name):
    """ Removes an auxillary factory coordinate from your cube:
    inputs:
       cube = cube to modify
       coord_name = auxillary factory coordinate to remove

    motivation for this routine:
       Sometimes cubes contains a problematic auxillary coordinate called
       'altitude' of size 1. This is generated by an auxillary factory where
       altitude is calculated from orography (which is ofen not supplied).
       This can cause problems in further calculations such as RMS and curl.
       Therefore this is removed here.
    """

    try:
        if cube.coords(coord_name)[0].points.size == 1:
            for factory in cube.aux_factories:
                if factory.name() == coord_name:
                    cube.remove_aux_factory(factory)
    except:
        pass


def remove_scalar_coord(cube, coord_name):
    """ Removes a scalar coordinate from your cube:
    inputs:
       cube = cube to modify
       coord_name = scalar coordinate to remove
    """

    coord_name_list = [coord.name() for coord in cube.coords()]
    for cubes_coord_name in coord_name_list:
        if cube.coords(cubes_coord_name)[0].points.size == 1:
            if cubes_coord_name == coord_name:
                cube.remove_coord(coord_name)


def add_scalar_coord(cube, coord_name):
    """ Adds a scalar coordinate to your cube:
    inputs:
       cube = cube to modify
       coord_name = (strring) scalar coordinate to add.
                    Currently only 'level_height' is acceptable.
    """

    acceptable_list = ['altitude', 'model_level_number', 'level_height']
    need_new_coord = True
    coord_name_list = [coord.name() for coord in cube.coords()]
    for cubes_coord_name in coord_name_list:
        if cubes_coord_name in acceptable_list:
            need_new_coord = False

    if need_new_coord:
        new_coord = iris.coords.DimCoord([20.0],
                                         standard_name=None,
                                         long_name=coord_name,
                                         units='m',
                                         bounds=np.asarray([0.0, 36.7]),
...
def quiver(u_cube, v_cube, scale=None):
    ...
    # If x_shrink has looped round the globe then shift it back
    if x_shrink[-1] > 360.0:
        x_shrink -= 360.0
    ...
    # Get the cartopy projection which we will use to transform the quiver plot
    proj = u_cube.coord_system().as_cartopy_projection()

    plt.quiver(x_shrink, y_shrink, u_trans, v_trans,
               scale_units='inches', scale=scale, pivot='tail', transform=proj)
...
def get_land_frac(incube):
    ...
    # Regrid it
    land_frac = regrid_cube(land_frac, incube, scheme_str='Linear')

    return land_frac


def is_regional(incube):
    """Test to see if a cube is regional"""

    # Default is false
    is_regional_bool = False
    is_regional_lon = False
    is_regional_lat = False
    ...
    if 'longitude' in coords_list:
        lon_name = 'longitude'
    if 'grid_longitude' in coords_list:
        lon_name = 'grid_longitude'

    if lon_name != 'none':
        if len(incube.coord(lon_name).points) > 1:
            # Does it not go round the globe (with a 10 degree margin)
            if incube.coord(lon_name).points[-1] - \
               incube.coord(lon_name).points[0] < 350:
                is_regional_lon = True

    # Find the latitude coordinate
    lat_name = 'none'
    if 'latitude' in coords_list:
        lat_name = 'latitude'
    if 'grid_latitude' in coords_list:
        lat_name = 'grid_latitude'

    if lat_name != 'none':
        if len(incube.coord(lat_name).points) > 1:
            # Does it not go from pole to pole (with a 50 degree margin)
            if abs(incube.coord(lat_name).points[-1] -
                   incube.coord(lat_name).points[0]) < 130:
                is_regional_lat = True

    # If any are regional then return True
    if is_regional_lon or is_regional_lat:
        print 'Detecting regional data'
        is_regional_bool = True

    return is_regional_bool
