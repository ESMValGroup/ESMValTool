...
def create_run_object(args, area, suite_id):
    ...
    run.period = '{:04d}_{:03d}'.format(run['start'], run['nyear'])  # stratosphere uses this
...
def parse_args(args):
    ...
    parser.add_argument('--suite-id1', required=True,
                        help='The name of a Rose suite, something like: u-ab123')
    parser.add_argument('--suite-id2', required=True,
                        help='The name of a Rose suite, something like: u-ab123')
    ...
    parser.add_argument('--ancil-dir', required=True,
                        help='Directory with Ancillary files.')
    parser.add_argument('--out-dir', required=True,
                        help='Write results into this directory.')
    parser.add_argument('--data-dir', required=True,
                        help='Directory tree with climate data.')
    ...
    # climatologic years
    regex = '^[0-9]{4}/12/01$'
    assert re.match(regex, args.start_date), regex + ' does not match ' + str(args.start_date)
    assert re.match(regex, args.end_date),   regex + ' does not match ' + str(args.end_date)
    assert args.start_date < args.end_date, 'The start date must be before the end date.'

    regex = '^(/[a-zA-Z0-9_-]*)+$'
    assert re.match(regex, args.obs_dir),   regex + ' does not match ' + str(args.obs_dir)
    assert re.match(regex, args.ancil_dir), regex + ' does not match ' + str(args.ancil_dir)
    assert re.match(regex, args.out_dir),   regex + ' does not match ' + str(args.out_dir)
    assert re.match(regex, args.data_dir),  regex + ' does not match ' + str(args.data_dir)
    assert re.match(regex, args.tmp_dir),   regex + ' does not match ' + str(args.tmp_dir)
...
def run_area():
    ...
    area_tmp_dir = create_tmp_dir(args.tmp_dir, args.suite_id1, args.suite_id2, area)
    area_out_dir = create_output_tree(args.out_dir, args.suite_id1, args.suite_id2, area)
    ...
    if hasattr(area_package, 'multi_functions'):
        ...
        run_objects = [ref_run_object, test_run_object]  # reference must be first
