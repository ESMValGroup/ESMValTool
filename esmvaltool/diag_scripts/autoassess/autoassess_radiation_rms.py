...
def apply_rms(data_1, data_2, cfg, component_dict, var_name):
    ...
    rms.calc_all(rms_list, data1_vs_data2, landsea_mask_cube,
                 plot_title, filename)
...
def main(cfg):
    ...
    for short_name in grouped_input_data:
        ...
        obs_selection = select_metadata(
            input_data, short_name=short_name, dataset=VAR_DICT[short_name])
        ...
        ctrl_sm, exper_sm, obs_sm = apply_supermeans(ctrl, exper, obs)
        ...
        if obs_sm:
            ...
            apply_rms(ctrl_sm, obs_sm, cfg, data_component_dict['ct-obs'],
                      short_name)
            # exper-obs
            logger.info("Computing EXPERIMENT-OBS RMS...")
            apply_rms(exper_sm, obs_sm, cfg, data_component_dict['ex-obs'],
                      short_name)
        else:
            # only ctrl-exper
            logger.info("Computing CONTROL-EXPERIMENT RMS...")
