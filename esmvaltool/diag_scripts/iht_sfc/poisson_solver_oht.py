...
"""Poisson solver for the ocean column.

The Poisson equation is solved by numerically using the bi-conjugate
gradient stabilized (BiCGSTAB) method.

To ensure a solution over the ocean only, Neumann boundary conditions are
...
Convergence is achieved faster by using a pre-conditioner on the output field.

The heat transport is calculated as the gradient of the energy flux potential,
the output of the Poisson solver.  This is done using a central difference
