...
def calculate_moving_average_centered(a, window_size):
    ...
    return pd.Series(a).rolling(window=window_size,
...
def calculate_gwl_exceedance_years(input_data_sorted, gwls, window_size):
    ...
    for data in input_data_sorted:
        ...
        logger.info('Processing %s %s %s %s %s ', project, exp, dataset,
                    ensemble)
        anomaly_ts_cube = iris.load_cube(data['filename'])
        years = anomaly_ts_cube.coord('year').points
        start_year = years[0]
        end_year = years[len(years) - 1]
        logger.info('Start and end years and averaging over  %s %s %s ',
                    str(start_year), str(end_year), str(window_size))
...
def main(cfg):
    ...
    gwl_df = calculate_gwl_exceedance_years(input_data_sorted, gwls,
                                            window_size)
