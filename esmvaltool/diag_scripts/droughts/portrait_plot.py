...
group_by: str, optional
    ...
    grouping into subplots. By default 'project'.
...
nan_color: str or None, optional
    Matplotlib named color or hexcode for NaN values. If set to None (null in
    yaml), no triangles are plotted for NaN values.
    By default 'white'.
...
regions: list(str), optional
    If list of acronyms is provided, data is limited to this
    regions (rather than global).
"""
...
def open_file(cfg, metadata, **selection):
    ...
    if cfg.get("regions", False):
        print("Using iris/esmvalcore to extract regions")
        cube = iris.load_cube(metas[0]["filename"])
        cube = pp.extract_shape(
            cube, shapefile="ar6", ids={"Acronym": cfg["regions"]}
        )
        das = xr.DataArray.from_iris(cube).to_dataset()
    else:
...
def load_data(cfg, metas):
    ...
    coords = {  # order matters: x, y, group, split
        ...
        cfg["split_by"]: [
            str(k) for k in group_metadata(metas, cfg["split_by"]).keys()
        ],
    }
    print(coords)
    ...
    for coord_tuple in itertools.product(*coords.values()):
        ...
        print(selection)
        data["var"].loc[selection] = open_file(cfg, metas, **selection)
...
def plot(cfg, data):
    ...
    for i in range(group_count):
        log.debug("Plotting group %s", i)
        print("Plotting group %s", i)
...
def normalize(array, method, dims):
    ...
    log.debug("Normalizing data with method %s", method)
    shift = 0
    norm = 1
    if "mean" in method:
...
def sort_data(cfg, dataset):
    ...
    if cfg.get("y_order"):
        # order = cfg["y_order"]
        # dataset[cfg["y_by"]] = pd.Categorical(
        #     dataset[cfg["y_by"]], categories=order, ordered=True
        # )
        dataset = dataset.reindex({cfg["y_by"]: cfg["y_order"]})
    # new_order = [dataset[cfg["x_by"]].values
    # dataset = dataset.reindex()
    return dataset


def apply_distance_metric(cfg, metas):
    """Optionally apply preproc method. reference_for_metric facet required."""
    if not cfg["distance_metric"]:
        return
    new_metas = []
    for y_metas in group_metadata(metas, cfg["y_by"]).values():
        references = select_metadata(y_metas, reference_for_metric=True)
        for ref_meta in references:
            ref_cube = iris.load_cube(ref_meta["filename"])
            for meta in y_metas:  # usually variable
                if meta.get("reference_for_metric", False):
                    continue  # skip distance to itself or other references
                cube = iris.load_cube(meta["filename"])
                # TODO: fix units should be done in preproc or the diagnostics
                if cube.units != ref_cube.units:
                    pp.convert_units(cube, ref_cube.units)
                distance = pp.distance_metric(
                    [cube], reference=ref_cube, metric=cfg["distance_metric"]
                )
                # basename = f"{Path(meta['filename']).stem}"
                split = ref_meta.get(cfg["split_by"], cfg["default_split"])
                basename = ""
                basename += f"_{meta[cfg['group_by']]}"
                basename += f"_{split}"
                basename += f"_{meta[cfg['y_by']]}"
                basename += f"_{meta[cfg['x_by']]}_{cfg['distance_metric']}"
                ...
                new_meta = meta.copy()  # duplicate for multiple references
                new_meta[cfg["split_by"]] = split
...
def main(cfg):
    ...
    set_defaults(cfg)
    print(cfg["distance_metric"])
    metas = apply_distance_metric(cfg, metas)
