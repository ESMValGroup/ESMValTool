...
def plot_bias_plot(data_list : list[Data4Analyis], cfg: dict):
    ...
    for data in data_list:
        ...
        data_col = eplot.get_dataset_style(
                                       data.name, cfg.get('color_style'))
        iris.plot.plot(data.best_guess, color=data_col['color'])
        iris.plot.fill_between(data.best_guess.dim_coords[0], data.border1,
                               data.border2, alpha = 0.2, linewidth=0, 
