...
XR_hist=xr.Dataset(data_vars=dict(velocity_mean_historical=(['time','depth'],np.mean(AR_hist,axis=0)),
                                  velocity_std_historical= (['time','depth'],np.std(AR_hist,axis=0)),
                                  velocity_min_historical= (['time','depth'],np.min(AR_hist,axis=0)),
                                  velocity_max_historical= (['time','depth'],np.max(AR_hist,axis=0))),
                   coords=dict(time=years_hist,
                               depth=depth),
                   attrs=dict(units='m/s',
                              description='annual averaged Atlantic Equatorial Undercurrent (AEU) velocity profile along depth and related sttistics (mean value, std, min, max), from averaged CMIP6 ensemble, historical simulations (1950-2015), values calculated at the equator between 0 and 500m. values are derived by first averaging all ensemble members for each CMIP6 model and then taking the average of all models.',
                              author='Giovanni Galli, Plymouth Marine Laboratory, 2022'))

XR_ssp=xr.Dataset(data_vars=dict(velocity_mean_ssp126=(['time','depth'],np.mean(AR_ssp126,axis=0)),
                                 velocity_std_ssp126= (['time','depth'],np.std(AR_ssp126,axis=0)),
                                 velocity_min_ssp126= (['time','depth'],np.min(AR_ssp126,axis=0)),
                                 velocity_max_ssp126= (['time','depth'],np.max(AR_ssp126,axis=0)),
                                 velocity_mean_ssp245=(['time','depth'],np.mean(AR_ssp245,axis=0)),
                                 velocity_std_ssp245= (['time','depth'],np.std(AR_ssp245,axis=0)),
                                 velocity_min_ssp245= (['time','depth'],np.min(AR_ssp245,axis=0)),
                                 velocity_max_ssp245= (['time','depth'],np.max(AR_ssp245,axis=0)),
                                 velocity_mean_ssp370=(['time','depth'],np.mean(AR_ssp370,axis=0)),
                                 velocity_std_ssp370= (['time','depth'],np.std(AR_ssp370,axis=0)),
                                 velocity_min_ssp370= (['time','depth'],np.min(AR_ssp370,axis=0)),
                                 velocity_max_ssp370= (['time','depth'],np.max(AR_ssp370,axis=0)),
                                 velocity_mean_ssp585=(['time','depth'],np.mean(AR_ssp585,axis=0)),
                                 velocity_std_ssp585= (['time','depth'],np.std(AR_ssp585,axis=0)),
                                 velocity_min_ssp585= (['time','depth'],np.min(AR_ssp585,axis=0)),
                                 velocity_max_ssp585= (['time','depth'],np.max(AR_ssp585,axis=0))),
                   coords=dict(time=years_scen,
                               depth=depth),
                   attrs=dict(units='m/s',
                              description='annual averaged Atlantic Equatorial Undercurrent (AEU) velocity profile along depth and related sttistics (mean value, std, min, max), from averaged CMIP6 ensemble, scenario simulations (SSP1 2.6, SSP2 4.5, SSP3 7.0, SSP5 8.5, 2015-2100), values calculated at the equator between 0 and 500m. values are derived by first averaging all ensemble members for each CMIP6 model and then taking the average of all models.',
                              author='Giovanni Galli, Plymouth Marine Laboratory, 2022'))


OUTDIR='/home/users/gig/CRACAB/FINAL_PLOT/'
OUTFILE_HIST=OUTDIR+'AEU_velocity_at_depth_historical_1950-2014.nc'
OUTFILE_SSP=OUTDIR+'AEU_velocity_at_depth_scenarios_1950-2014.nc'
