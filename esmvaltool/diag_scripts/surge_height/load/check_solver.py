...
def check_solver(eof_inpath):
    ...
    gradpsl_solver = os.path.join(
        eof_inpath, 'eof_gradpsl_solver_075x075_19790101-20151231.pkl')
    ...
    if (os.path.exists(psl_solver) and os.path.exists(gradpsl_solver)
            and os.path.exists(uas_solver) and os.path.exists(vas_solver)):
        for file in [psl_solver, gradpsl_solver, uas_solver, vas_solver]:
            try:
