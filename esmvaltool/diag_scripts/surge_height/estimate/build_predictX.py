def build_predictX(dates, pseudo_pcs_SLP, pseudo_pcs_gradlatSLP,
    ...
    X[0].extend(
        [m1day_wgt * x for x in pseudo_pcs_gradlatSLP[0].tolist()[:xnpcs]])
    X[0].extend(
        [m1day_wgt * x for x in pseudo_pcs_gradlonSLP[0].tolist()[:xnpcs]])
    ...
    for t in range(0, len(dates) - 1):
        ...
        X[t + 1].extend(
            [m1day_wgt * x for x in pseudo_pcs_gradlatSLP[t].tolist()[:xnpcs]])
        X[t + 1].extend(
            [m1day_wgt * x for x in pseudo_pcs_gradlonSLP[t].tolist()[:xnpcs]])
        X[t + 1].extend(
