def build_predictX(dates, pseudo_pcs_SLP, pseudo_pcs_gradSLP,
                   pseudo_pcs_u, pseudo_pcs_v):
    m1day_wgt = 0.5
    m2day_wgt = 0.25
    xnpcs = 1000
    #
    ...
    X[0].extend([m2day_wgt*x for x in pseudo_pcs_gradSLP[0].tolist()[:xnpcs]])
    ...
    X[1].extend([m2day_wgt*x for x in pseudo_pcs_gradSLP[1].tolist()[:xnpcs]])
    ...
    for t in range(0,len(dates)-2):
    	...
    	X[t+2].extend([m2day_wgt*x for x in pseudo_pcs_gradSLP[t].tolist()[:xnpcs]])
