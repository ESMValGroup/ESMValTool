...
class LifetimeBase:
    ...
    def __init__(self, config):
        ...
        default_config = os.path.join(
            os.path.dirname(__file__), "lifetime_config.yml"
        )
        # cartopy_data_dir = config.get('cartopy_data_dir', None)
        # if cartopy_data_dir:
        #     cartopy.config['data_dir'] = cartopy_data_dir
        with open(
            config.get("config_file", default_config), encoding="utf-8"
        ) as config_file:
    ...
    def get_plot_folder(self, var_info):
        ...
        folder = os.path.expandvars(
            os.path.expanduser(list(_replace_tags(self.plot_folder, info))[0])
        )
