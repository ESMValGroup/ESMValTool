...
def get_path_to_mpl_style(style_file):
    ...
    filepath = os.path.join(base_dir, 'styles_python', 'matplotlib',
                            style_file)
    logger.debug("Using matplotlib style: {}".format(filepath))
    return filepath
...
def get_model_style(model, style_file='default'):
    ...
    if os.path.isfile(filepath):
        ...
        raise IOError("Invalid input: could not open style file " +
                      "'{}'".format(filepath))
    logger.debug("Using style file {} for model {}".format(filepath, model))
    ...
    for option in options:
        if (styleconfig.has_option(model, option)):
            ...
            logger.warning("No style information '{}' ".format(option) +
                           "found for model '{}', ".format(model) +
    ...
    if (styleconfig.has_section(model)):
        ...
        logger.warning("No style information for model '{}' ".format(model) +
                       "available")
