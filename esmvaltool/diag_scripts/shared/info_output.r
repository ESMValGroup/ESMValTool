...
write_integer <- function (indent, filename){
    file_conn <- file(filename)
    writeLines(as.character(indent), file_conn)
    close(file_conn)
}
...
info_output <- function(output_string,
    ...
    if (nchar(main_wd) == 0) {
        ...
        if (verbosity >= required_verbosity) {
            entering_routine <- regexpr("<<<<<<<< Entering",
                                        output_string, fixed = T)[1]
            ...
            pasted_string <- paste("info: ", indent_str, output_string,
                                   sep = "")
            print(pasted_string)

            # Decrease indentation if we're leaving an NCL routine
            leaving_routine <- regexpr(">>>>>>>> Leaving", output_string,
                                       fixed = T)[1]
