...
def main(project_info):
    ...
    for model in project_info['MODELS']:
        ...
        sftlf_fx[model_name] = currProject.get_cf_lmaskfile(project_info, 
                                                            model)
    model_names.sort()
    modeltags = ['M_' + item for item in model_names]

    modelconfig = ConfigParser.ConfigParser()
    modelconfig.read(config_file)
    # Get CMOR names of variables we are going to use
    currVars = E.get_currVars()
    vartags = ['V_' + item for item in currVars]
    # Create a dict that can be queried in a way
    # filenames[variable][model]
    # result is path to the ESMValTool processed file
    filenames = {}
    for vvar in currVars:
        filenames[vvar] = E.get_clim_model_filenames(variable=vvar, 
                                                     monthly=False)
    plot_tags = basetags + modeltags + vartags

    dataIDs = 'CMIP5'
    authors = 'A_lemb_va'
    cfg_options = {'eb':'n', 'lsm':'n','wat':'n','lec':'n','entr':'n','met':'n'}
    cfg = E.get_config_options(modelconfig, 'options', cfg_options)
    eb=cfg['eb']
    lsm=cfg['lsm']
    wat=cfg['wat']
    lec=cfg['lec']
...
#
#    log.write('Options specified by the user (y or yes, n or no):\n')
#    eb  = str(raw_input('Do you want me to compute energy budgets? '))
#    log.write('Energy budget is computed: {}\n'.format(eb))
#    lsm = str(raw_input(
#            'Do you want me to perform computations on land and oceans separately (when applicable)? '))
#    log.write('Results on land and ocean separately: {}\n'.format(lsm))
#    wat = str(raw_input('Do you want me to compute water mass budget? '))
#    log.write('Water mass budget is computed: {}\n'.format(wat))
#    lec = str(raw_input('Do you want me to compute the Lorenz Energy Cycle? '))
#    log.write('Lorenz Energy Cycle is computed: {}\n'.format(lec))
#    entr = str(raw_input('Do you want me to compute material entropy production?'))
#    log.write('Material Entropy Production is computed: {}\n'.format(entr))
#    if entr in {'y', 'yes'}:
#        met = str(raw_input('Which method (1: indirect, 2: direct, 3: both)? '))
#        log.write('With method (1: indirect, 2: direct, 3: both): {}\n'
#                  .format(met))
#    else:
#        met = 0
#        pass
#    log.write('Done\n')
    ...
    for model_name in model_names:
        ...
        plotpath2   = os.path.join(plotpath, model_name)
        ...
        if not os.path.exists(plotpath2):
            ...
            for root, dirs, files in os.walk(plotpath2):
                ...
                for name in dirs:
                    ...
                    #os.rmdir(file_path)
        print(model_name)
        log.write('Processing model: {}\n'.format(model_name))

        pr_file    = filenames['pr'][model_name]
        prsn_file  = filenames['prsn'][model_name]
        ps_file    = filenames['ps'][model_name]
        rsdt_file  = filenames['rsdt'][model_name]
        rsut_file  = filenames['rsut'][model_name]
        rlut_file  = filenames['rlut'][model_name]
        rsds_file  = filenames['rsds'][model_name]
        rsus_file  = filenames['rsus'][model_name]
        rlds_file  = filenames['rlds'][model_name]
        rlus_file  = filenames['rlus'][model_name]
        hfls_file  = filenames['hfls'][model_name]
        hfss_file  = filenames['hfss'][model_name]
        ts_file    = filenames['ts'][model_name]
        uas_file   = filenames['uas'][model_name]
        vas_file   = filenames['vas'][model_name]
        tas_file   = filenames['tas'][model_name]
        hus_file   = filenames['hus'][model_name]
        ta_file    = filenames['ta'][model_name]
        ua_file    = filenames['ua'][model_name]
        va_file    = filenames['va'][model_name]
        wap_file   = filenames['wap'][model_name]

        aux_file = diagworkdir + '/{}_aux.nc'.format(model_name)

        log.write('Computing auxiliary variables\n')
        ...
        log.write('Global mean emission temperature: {}\n'
        ...
        cdo.monmean(input = tas_file, options = '-b F32', output = tasmean_file)
        ...
        cdo.mermean(input = '-zonmean -mulc,0.5 -add {} {}'.format(ts_file,
                     tasmean_file), options = '-b F32', output = tasvert_file) 
        # evaporation
        ...
        if entr in {'y','yes'}:
				...
				options = '-b F32', output = tcloud_file)
                    ...
                    removeif(tasvert_file)
                    cdo.mermean(input='-zonmean -mulc,0.5 -add {} {}'
        ...
        #Energy budgets
        if eb in {'y','yes'}:
            log.write('Computing energy budgets\n')
            #TOA energy budget
            ...
            log.write('Surface energy budget: {}\n'.format(surb_all[i,0]))
            ...
            log.write('Atmospheric energy budget: {}\n'.format(atmb_all[i,0]))
            log.write('Done\n')
            ...
            log.write('Baroclinic efficiency (Lucarini et al., 2011): {}\n'.format(barocEff))
        	...
        	#Water mass budget
        if wat in {'y','yes'}:
            log.write('Computing water mass and latent energy budgets\n')
            ...
            log.write('Latent energy budget: {}\n'.format(latent_all[i,0]))
            log.write('Done\n')
        else:
        	    pass
        ...
        if lsm in {'y','yes'}:	
            if eb in {'y','yes'}:
                log.write('Computing energy budgets over land and oceans\n')
                ...
                log.write('TOA energy budget over oceans: {}\n'
                ...
                log.write('TOA energy budget over land: {}\n'.format(toab_la_gmean_constant))
                ...
                log.write('Atmospheric energy budget over oceans: {}\n'.format(atmb_oc_gmean_constant))
                ...
                log.write('Atmospheric energy budget over land: {}\n'.format(atmb_la_gmean_constant))
                ...
                log.write('Surface energy budget over oceans: {}\n'.format(surb_oc_gmean_constant))
                ...
                log.write('Surface energy budget over land: {}\n'.format(surb_la_gmean_constant))
            ...
            if wat in {'y','yes'}:
                log.write('Computing water mass and latent energy budgets over land and oceans\n')
                toab_ocean_file = diagworkdir+'/{}_toab_ocean.nc'.format(model_name)
                ...
                log.write('Water mass budget over oceans: {}\n'.format(wmass_oc_gmean_constant))
                ...
                log.write('Water mass budget over land: {}\n'.format(wmass_la_gmean_constant))
                ...
                log.write('Latent energy budget over oceans: {}\n'.format(latent_oc_gmean_constant))
                ...
                log.write('Latent energy budget over land: {}\n'.format(latent_la_gmean_constant))
        ...
        if lec in {'y','yes'}: 
            log.write('Computation of the Lorenz Energy Cycle (year by year)\n')
...
#                tainput = diagworkdir+'/tainput.nc'
#                cdo.setmisstoc('0',input='-selyear,{} -invertlat {}'.format(str(yr),ta_file), 
#                            options = '-b F32', output=tainput)
#                uainput = diagworkdir+'/uainput.nc'
#                cdo.setmisstoc('0',input='-selyear,{} -invertlat {}'.format(str(yr),ua_file), 
#                            options = '-b F32', output=uainput)
#                vainput = diagworkdir+'/vainput.nc'
#                cdo.setmisstoc('0',input='-selyear,{} -invertlat {}'.format(str(yr),va_file), 
#                            options = '-b F32', output=vainput)
#                wapinput = diagworkdir+'/wapinput.nc'
#                cdo.setmisstoc('0',input='-selyear,{} -invertlat {}'.format(str(yr),wap_file), 
#                            options = '-b F32', output=wapinput)
#                tasinput=diagworkdir+'/tasinput.nc'
#                            options = '-b F32', output=tainput)
#                yr = int(filter(str.isdigit,yr))
#                tainput = diagworkdir+'/tainput.nc'
...
#                uainput = diagworkdir+'/uainput.nc'
#                cdo.selyear(yr,input='-invertlat {}'.format(ua_file), 
#                            options = '-b F32', output=uainput)
#                vainput = diagworkdir+'/vainput.nc'
#                cdo.selyear(yr,input='-invertlat {}'.format(va_file), 
#                            options = '-b F32', output=vainput)
#                wapinput = diagworkdir+'/wapinput.nc'
#                cdo.selyear(yr,input='-invertlat {}'.format(wap_file), 
#                            options = '-b F32', output=wapinput)
                #tasinput = diagworkdir+'/taainput.nc'
                #cdo.selyear(yr, input = tas_file, options = '-b F32', 
                #            output = tasinput)
                fourc.fourier_coeff('inputen.nc')
                #cmd = 'ncl -n -Q fourier_coefficients_new.ncl'
                ...
                #lect[y] = lecscript.lorenz(auxpath, diagworkdir, plotpath, model_name,
                #                            yr, tainput, uainput, vainput, wapinput, 
                #                            tasinput, diagfile, logfile)
                lect[y] = lecscript.lorenz(auxpath, diagworkdir, plotpath, model_name,
        ...
        if entr in {'y', 'yes'}:
            if met in {'1', '3'}:
                log.write('Computation of the material entropy production with the indirect method\n')
                ...
                cdo.mermean(input = '-zonmean {}'.format(horizEntropy_file),
                ...
                log.write('Horizontal component of the material entropy production: {}\n'.format(horzentr_all[i,0]))
                ...
                log.write('Vertical component of the material entropy production: {}\n'.format(vertentr_all[i,0]))
                log.write('Done\n')
            if met in {'2','3'}:
                log.write('Computation of the material entropy production with the direct method\n')
                log.write('1. Sensible heat fluxes\n')
                tasmean_file = diagworkdir + '/{}_tmean.nc'.format(model_name)
                removeif(tasmean_file)
                ...
                cdo.mermean(input = '-zonmean {}'.format(sensentr_file),
                            options = '-b F32',output = sensentr_mean_file)
                fl = Dataset(sensentr_mean_file)
                sensentr_mean = fl.variables['ssens'][0, 0, 0]
                log.write('Material entropy production associated with sens. heat fluxes: {}\n'.format(sensentr_mean))
                log.write('Done\n')
                ######################################################################################################### 
                log.write('2. Hydrological cycle\n')
                log.write('2.1 Evaporation fluxes\n')
                ...
                ##############################################################################################
                log.write('2.2 Rainfall precipitation\n')
                ...
                cdo.mermean(input = '-zonmean {}'.format(rainentr_file),
                            options = '-b F32',output = rainentr_mean_file)
                fl = Dataset(rainentr_mean_file)
                rainentr_mean = fl.variables['srain'][0, 0, 0]
                log.write('Material entropy production associated with rainfall: {}\n'.format(rainentr_mean))
                log.write('Done\n')
                ##############################################################################################
                log.write('2.3 Snowfall precipitation\n')
                ...
                cdo.mermean(input = '-zonmean {}'.format(snowentr_file), 
                            options = '-b F32', output = snowentr_mean_file)
                fl = Dataset(snowentr_mean_file)
                snowentr_mean = fl.variables['ssnow'][0, 0, 0]
                log.write('Material entropy production associated with snowfall: {}\n'.format(snowentr_mean))
                log.write('Done\n')
                ##############################################################################################
                log.write('2.4 Phase changes from ice to rain\n')
                ...
                cdo.timmean(input = '-mermean -zonmean {}'
                ...
                log.write('Material entropy production associated with phase changes from ice to rain: {}\n'.format(phir_entr_mean))
                log.write('Done\n')
                ##############################################################################################
                log.write('2.5 Phase changes from water vapor to snow\n')
                ...
                cdo.mermean(input = '-zonmean {}'.format(phvs_entr_file), 
                            options='-b F32', output = phvs_entr_mean_file)
                fl = Dataset(phvs_entr_mean_file)
                phvs_entr_mean = fl.variables['slatps'][0, 0, 0]
                log.write('Material entropy production associated with phase changes from vapor to snow: {}\n'.format(phvs_entr_mean))
                log.write('Done\n')
                ##############################################################################################
                log.write('2.6 Potential energy of the droplet\n')
                ...
                cdo.mermean(input = '-zonmean {}'.format(potentr_file), 
                            options = '-b F32', output = potentr_mean_file)
                fi = Dataset(potentr_mean_file)
                potentr_mean = fi.variables['spotp'][0, 0, 0]
                log.write('Material entropy production associated with potential energy of the droplet: {}\n'.format(potentr_mean))
                log.write('Done\n')
                ##############################################################################################
                log.write('3. Kinetic energy dissipation\n')
                if lec in {'y','yes'}: 
                    ...
                    log.write('Material entropy production associated with kinetic energy dissipation: {}\n'.format(minentr_mean))
                else:
                    minentr_mean = 0.010
                    log.write('I cannot compute the material entropy production without the LEC...\n')
                    log.write('I will assign a given value for the material entropy production attributed to LEC (0.01 W/m2*K)\n')
                ###########################################################################################
                ...
                log.write('Material entropy production with the direct method: {}\n'.format(matentr))
                matentr_all[i,0] = matentr
                if met in {'3'}:
                    diffentr = float(np.nanmean(vertentr_mean)) + float(np.nanmean(horzentr_mean)) - matentr
                    log.write('Difference between the two methods: {}\n'.format(diffentr))
                ...
                log.write('Degree of irreversibility of the system: {}\n'.format(irrevers))
        ...
        log.write('Running the plotting module for the budgets\n')
        if eb in {'y','yes'}:
            plotsmod.balances(diagworkdir_up, plotpath2, 
                              [toab_file, atmb_file, surb_file],
                              sftlf_fx[model_name], ['toab','atmb','surb'],
                              model_name, lsm)
	    ...
	    dataIDs = "hfls, hfss, rlds, rlus, rlut, rsds, rsdt, rsus, rsut (time res: monthly, vertical: 2D TOA/surf)" 
            ...
            log.write('Done\n')
        if wat in {'y','yes'}:
            log.write('Running the plotting module for the water budgets\n')         
            plotsmod.balances(diagworkdir_up, plotpath2,
                              [wmassBudget_file, latentEnergy_file],
                              sftlf_fx[model_name], ['wmass','latent'], 
                              model_name, lsm)               
        ...
        if entr in {'y','yes'}:
            if met in {'1'}:
                log.write('Running the plotting module for the material entropy production (indirect method)\n')
	        ...
	        dataIDs = "rlut, rsdt rsut (time res: monthly, vertical: 2D TOA)" 
                ...
                log.write('Running the plotting module for the material entropy production (direct method)\n')
            ...
            pass
        log.write('Done for model: {} \n'.format(model_name))
        i = i+1
    
    info('I will now start multi-model plots', verbosity, 1)
    if eb in {'y', 'yes'}:   
        ...
        plt.xlim(-90, 90)
        plt.grid()
        oname=plotpath + 'meridional_transp.png'
        plt.savefig(oname)
        plt.show(fig)
        plt.close(fig)
    ...
    info('Scatter plots', verbosity,1)
    ...
    oname = plotpath + 'scatters_summary.png'
    plt.savefig(oname)
    plt.subplots_adjust(hspace = .3)
    ...
    info('Scatter plots for inter-annual variability of some quantities', 
         verbosity, 1)
    ...
    plt.subplots_adjust(hspace = .3)
    plt.grid()
    plt.savefig(plotpath + 'scatters_variability.png')
    plt.show(fig)
    plt.close(fig)
    ...
    log.write("The diagnostic has finished. Now closing...\n")
