...
def plot_ts(cfg, plot_dict, title, y_title, output_basename):
    ...
    for dataset, dict_info in plot_dict.items():
        ...
        if dataset in obs_datasets:
            ...
            # Skip plotting non-highlight models while still using them for multimodel stats.
            continue
...
def plot_ts_gradient(cfg, plot_dict_west, plot_dict_east, title, y_title, output_basename):
    """
    ...
    for dataset in plot_dict_west.keys():
        if dataset not in plot_dict_east:
            logger.warning(f"Dataset {dataset} not found in east dict, skipping gradient.")
            continue
        ...
        input_filenames.update(file_west if isinstance(file_west, list) else [file_west])
        input_filenames.update(file_east if isinstance(file_east, list) else [file_east])
        ...
        if values_west.ndim != 1 or values_east.ndim != 1:
            logger.warning(f"Skipping {dataset}: expected 1D monthly data.")
            continue
    ...
    provenance_record = get_provenance_record(output_basename, list(input_filenames))
    save_figure(output_basename, provenance_record, cfg)
    logger.info(f"Monthly gradient plot saved: {output_basename}")
    plt.close()
