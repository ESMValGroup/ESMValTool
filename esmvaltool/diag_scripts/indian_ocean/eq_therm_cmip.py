...
import numpy as np
from basic_functions import get_provenance_record, iso_depth_4d, load_data
...
def extract_eq_IO(cube):
    ...
    io_inds = np.squeeze(
        np.where((lon_points >= IO_LON_MIN) & (lon_points <= IO_LON_MAX))
    )
    ...
    for i in range(4):
        pairs = sorted(zip(io_lons, io_t20d[i], strict=True))
        _sorted_lons, sorted_season = zip(*pairs, strict=True)
...
def get_iso_data(cfg, group_md, variable):
    """
    Compute T20D from 4D temperature fields and 
    return dataset-mean cube dictionary.
    """
    logger.info(
        f"Processing iso data for variable: {variable}"
    )
    iso_results = {}
    ...
    is_2d_coords = lon_coord.ndim == 2
    if is_2d_coords:
        print(
            f"{dataset} has 2D longitude coordinates, applying extract_eq_IO.",
        )
        eq_iso_data = extract_eq_IO(t20d_cube)
    else:
        print(
            f"{dataset} has 1D longitude coordinates, "
            f"applying longitude constraint.",
        )
        # Extract longitude range
        lon_constraint = iris.Constraint(
            longitude=lambda lon: IO_LON_MIN <= lon <= IO_LON_MAX,
        )
        ...
        eq_iso_data = [
            eq_iso_mean[i].data
            for i in range(len(eq_iso_mean.coord("season_number").points))
        ]

    iso_results[dataset] = {"cube": eq_iso_data, "filename": file}
...
def plot_ts(cfg, plot_dict, title, output_basename):
    ...
    for n, season in enumerate(seasons):
        ...
        mohc_idx = 0
        multimodel_lons = np.linspace(IO_LON_MIN, IO_LON_MAX, 300)
        multimodel_profiles = []
        ...
        for dataset, dict_info in plot_dict.items():
            ...
            input_filenames.update(file if isinstance(file, list) else [file])
            lons = np.linspace(IO_LON_MIN, IO_LON_MAX, np.shape(cube[n])[0])

            model_vals = np.asarray(cube[n], dtype=float)
            valid = np.isfinite(model_vals)
            if dataset != "EN4" and \
               np.count_nonzero(valid) >= MIN_VALID_POINTS:
            ...
            if dataset == "EN4":
                ...
                # Skip plotting non-MOHC model lines while still using them
                # for multimodel stats.
                continue
        ...
        plt.xlim(IO_LON_MIN, IO_LON_MAX)
...
def main(cfg):
    """
    Compute the 20 degree isotherm along the equatorial Indian Ocean and plot
    ...
    plot_ts(
        cfg,
        iso_results,
        "20 $^\\circ$C isotherm depth along the equatorial Indian Ocean",
        "eq_IO_t20d",
    )
