...
def as_scalar(cube, method="mean"):
    ...
    if method == "mean":
        ...
        return float(
            cube.collapsed(cube.dim_coords, iris.analysis.MEAN).data
        )
    if method == "std":
...
def build_bias_table(cfg, diagnostics):
    ...
    if not model_names:
        logger.warning(
            "No model diagnostics available for bias portrait plot.",
        )
        return
    ...
    # Standardize each diagnostic across models (z-score) 
    # so all columns share a comparable, 
    # unitless scale for plotting and table export.
    ...
    bias_cube = Cube(
        ...
        dim_coords_and_dims=[
            (
                DimCoord(np.arange(len(model_names)), long_name="model_index"),
                0,
            ),
            (
                DimCoord(
                    np.arange(len(metric_names)),
                    long_name="diagnostic_index",
                ),
                1,
            ),
        ],
    ...
    print(
        f"Saving bias cube with shape {bias_cube.shape} "
        f"and data:\n{bias_cube.data}",
    )  # Debug statement
    bias_cube.add_aux_coord(
        AuxCoord(np.array(model_names), long_name="model_name"),
        0,
    )
    bias_cube.add_aux_coord(
        AuxCoord(np.array(metric_names), long_name="diagnostic_name"),
        1,
    )
...
def plot_bias_heatmap(cfg, matrix, model_names, metric_names, ancestors):
    ...
    fig, (ax_main, ax_gap, ax_mean) = plt.subplots(
        ...
        gridspec_kw={
            "width_ratios": [len(metric_names), 0.8, 1.1],
            "wspace": 0.02,
        },
    )
    ...
    ax_main.set_xticklabels(metric_names, rotation=45, 
    ...
    # Spacer axis creates a visible gap between 
    # the main table and mean-z column.
    ax_gap.axis("off")
...
def get_iso_data(cfg, group_md, variable):
    """
    Compute T20D from 3D temperature fields and 
    return dataset-mean cube dictionary.
    """
    logger.info(f"Processing iso data for variable: {variable}")
    iso_results = {}

    ISO_LEVEL = 20  # Define the isotherm level for T20D
    FILL_VALUE = 1e20  # Define the fill value for masking
    ...
    # Collapse the cube
    t20d_mean = t20d_cube.collapsed(
        ["latitude", "longitude"],
        iris.analysis.MEAN,
    )
    print(
        f"Processed T20D for dataset: {dataset}, mean value: {t20d_mean.data}",
    )  # Debug statement
    iso_results[dataset] = {"cube": t20d_mean, "filename": file}
...
def scat_mean_vs_std(cfg, xy_dict, x_label, y_label, title, output_basename):
    ...
    for i, (dataset, xy_info) in enumerate(xy_dict.items()):
        ...
        marker = (
            "o" if dataset not in cfg.get("highlight_datasets", []) else "*"
        )
...
def find_matching_y_data(dataset, x_prefix, y_dict, colors, i):
    ...
    OBS_LABEL = "OBS"
    print(
        f"Finding match for dataset: {dataset} with prefix: {x_prefix}",
    )  # Debug statement

    if x_prefix.startswith(OBS_PREFIX):
        for _y_dataset, y_candidate in y_dict.items():
            y_file = y_candidate["filename"]
            y_file_base = (
                os.path.basename(y_file)
                if isinstance(y_file, str)
                else os.path.basename(y_file[0])
            )
            if y_file_base.startswith(x_prefix):
                print(
                    f"Found matching OBS entry of {y_candidate}",
                )  # Debug statement
                return y_candidate, OBS_LABEL, OBS_COLOUR
        # No matching OBS entry found
        logger.warning(
            f"No matching OBS entry found in y_dict for {dataset}.",
        )
        return None, OBS_LABEL, OBS_COLOUR

    if dataset in y_dict:
        print(
            f"Found matching dataset in {y_dict[dataset]}",
        )  # Debug statement
...
def scat_plot(cfg, x_dict, y_dict, x_label, y_label, title, output_basename):
    ...
    for i, (dataset, x_info) in enumerate(x_dict.items()):
        ...
        input_filenames.update(
            x_file if isinstance(x_file, list) else [x_file],
        )

        x_prefix = get_prefix(x_file)
        y_info, label, color = find_matching_y_data(
            dataset,
            x_prefix,
            y_dict,
            colors,
        ...
        if y_info:
            ...
            input_filenames.update(
                y_file if isinstance(y_file, list) else [y_file],
            )
...
def main(cfg):
    ...
    for group_name, group_md in grouped_data.items():
        ...
        west_sst_son_item = load_data(
            group_md,
            "west_sst_son",
            get_filenames=True,
        )
        east_sst_son_item = load_data(
            group_md,
            "east_sst_son",
            get_filenames=True,
        )
        ...
        if west_sst_son_item and east_sst_son_item:
            sst_grad_item = compute_cube_diff(
                cfg,
                west_sst_son_item,
                east_sst_son_item,
                "sst_grad",
