...
MIN_VALID_POINTS = 2  # Minimum number of valid points for statistics
MAX_VALID_VALUES = 1e10  # Maximum valid value for data points
...
def calculate_taylor_stats(model_cube, obs_cube):
    """
    Calculate Taylor diagram statistics for a 
    given model and observation cube.
    ...
    valid_mask = (
        ...
        & (model_data <= MAX_VALID_VALUES)
        & (obs_data <= MAX_VALID_VALUES)
    )
    ...
    if model_data.size < MIN_VALID_POINTS or obs_data.size < MIN_VALID_POINTS:
        return {
            "correlation": np.nan,
            "stddev": np.nan,
            "std_obs": np.nan,
...
def _separate_obs_and_models(plot_dict):
    """
    Split plot_dict metadata into obs entries, 
    model entries and file list.
    """
    input_filenames = set()
    obs_entries = {}
    model_entries = {}
        ...
        for info in entries.values():
            ...
            try:
                points = np.atleast_1d(
                    cube.coord("season_number").points
                ).astype(int)
...
def _plot_taylor_panel(
    ...
    ax.set_ylabel(
        "Normalised standard deviation ($\\sigma / \\sigma_{obs}$)",
        labelpad=20,
    )
...
def plot_taylor(cfg, plot_dict, title, output_basename):
    ...
    input_filenames, obs_entries, model_entries = _separate_obs_and_models(
        plot_dict
    )

    if not obs_entries:
        logger.warning(
            "No observations found for Taylor diagram, skipping %s",
            output_basename,
        )
        return
    if not model_entries:
        logger.warning(
            "No model datasets found for Taylor diagram, skipping %s",
            output_basename,
        )
        return
    ...
    if seasons:
        ...
        for idx, season_number in enumerate(seasons):
            season_label = SEASON_LABELS.get(
                int(season_number), f"Season {int(season_number)}"
            )
            _plot_taylor_panel(
                flat_axes[idx],
                obs_entries,
...
def main(cfg):
    """
    Create Taylor diagrams for seasonal and annual 
    Indian Ocean diagnostics.
    """
    ...
    io_t20d_seas = _create_iso_depth_dict(
        cfg, io_theta_seas, iso_level=20.0, time_measure="season_number"
    )
    io_t20d_annual = _create_iso_depth_dict(
        cfg, io_theta_annual, iso_level=20.0, time_measure=None
    )
