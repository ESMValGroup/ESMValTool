...
def plot_latlon_cubes(
    ...
    if not obs_name:
        ...
        for cube, cube_name in zip(cubes, cube_names, strict=False):
            if not season:
                plot_file_path = os.path.join(
                    cfg["plot_dir"],
                    "alltime",
...
def plot_ctrl_exper_seasons(ctrl_seasons, exper_seasons, cfg, plot_key):
    ...
    if cfg["analysis_type"] == "lat_lon":
        for control_season, experiment_season, season in zip(
            ctrl_seasons, exper_seasons, seasons, strict=False
        ):
            plot_latlon_cubes(
                control_season,
                experiment_season,
...
def main(cfg):
    ...
    for short_name in grouped_input_data:
        ...
        if obs_list:
            for obs_i, obsfile in zip(obs_list, obs, strict=False):
