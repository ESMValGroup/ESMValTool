...
gleckler_main <- function(path = "./",
  ...
  if (promptInput == "y") {
    ...
    #print(cmd)
    #system(cmd)
    unlink(c(paste0(time_cropped, "/*.nc"), paste0(regridded, "/*.nc"),
             paste0(land, "/*.nc")))
    ...
    for (idx in idx_list){
      ...
      for (mo in modelsAndObs) {
        ...
        regridAndLandSeaMask(idx_raw =paste(time_cropped, "/", mo, sep = ""),
    ...
    RMSErelarr <- gleckler_array(path = land, idx_list=idx_list,
...
gleckler_array <- function(path = land, idx_list=gleckler_idx,
  ...
  for (i in seq_along(idx_list)) {
    ...
    ensarr <- array(NA, dim=c(nrow(tm_model_idx), ncol(tm_model_idx),
    ...
    for (j in 1:dim(ensarr)[3]) {
      ...
      for (o in seq_along(obs_list)) {
        tm_obs <- nc_open(Sys.glob(file.path(path, paste0("tm_", idx_list[i],
                                                         "_", obs_list[o],
                                                         "*.nc"))))
        tm_obs_idx <- ncvar_get(tm_obs, idxs)
        nc_close(tm_obs)
        RMSEarr[i + 1, j, o] <- RMSE(model = ensarr[,, j], obs = tm_obs_idx,
        ...
        RMSEarr[i + 1, ncol(RMSEarr), o] <-
          sqrt(area.mean((tm_obs_idx - area.mean(tm_obs_idx, lat = model_lat)) ^ 2,
                         lat = model_lat))
      }
    }
  }
...
gleckler_plotting <- function(arr = RMSErelarr, idx_list, model_list,
  ...
  if (!(nobs %in% c(1, 2, 3, 4))) {
    if (nobs == 0){
      ...
      print(paste("Too many reanalysis datasets provided.",
  ...
  figure_filename <- paste(plot_dir, "/Gleckler_", MIP_name, "_",
                           ...
                           syear, "-", eyear, ".",output_file_type, sep="")
  ...
  #gl_RMSEspacer <- 0.01
  plot(x = c(0, 1 + gl_RMSEspacer), y = c(0, 1), type = "n", ann = FALSE,
  ...
  for (yj in 2:yn) {
    for (zk in 1:nobs) {
      polygon(x = (xs[[zk]] / xn) + ( (xn - 1) / xn) + gl_RMSEspacer,
              y = (ys[[zk]] / yn) + ( (yn - yj) / yn),
  ...
  par(xpd = TRUE)
  rect(xleft = 0, ybottom = 0, xright = (1 - 1 / xn), ytop = (1 - 1 / yn),
       density = NULL, angle = 45,
       col = NA, border = 1, lty = par("lty"), lwd = 4)
  rect(xleft = 0, ybottom = (1 - 1 / yn), xright = (1 - 1 / xn), ytop = 1,
       density = NULL, angle = 45,
       col = NA, border = 1, lty = par("lty"), lwd = 4)
    ...
    for (a in 1:exlen){
      if (a == 1) {
        ...
        ytmp <- (scaling_factor * (sqrys / exlen + (a - 1) / exlen) +
                 yscale_spacer)
        polygon(x = xtmp, y = ytmp, col = glc[a])
        text(x = max(xtmp), y = max(ytmp), round(sclseq[a] + seq_shift, 1),
             cex = text.scaling_factor, pos = 4)
      }
    }
  }
  ...
  ## Plot scales
  gleckler_scale(sclseq, glc, scaling_factor = gl_scaling_factor,
                 text.scaling_factor = gl_text.scaling_factor,
                 xscale_spacer = gl_xscale_spacer_RMSE)
  
  gleckler_scale(sclseq_bw, glbw, scaling_factor = gl_scaling_factor,
                 text.scaling_factor = gl_text.scaling_factor,
  ...
  for (zk in 1:nobs) {
    ...
    text(x = xtxsym, y = ytxsym, labels = obs_list[zk], adj = 0.5,
         cex = gl_text.symb.scaling_factor, srt = rotx[zk])
  }
  ...
  axis(side = 1, at = xtcks1, labels = col_names, las = 2,
       cex.axis = axlabsize, tick = FALSE, line = lineadj)

  xtcks2 <- ( (xn - 1) / xn) + gl_RMSEspacer + (0.5 / xn)
  axis(side = 1, at = xtcks2, labels = expression("RMSE"["std"]),
...
#  source("nml/cfg_ExtremeEvents/cfg_climdex.r")
  ...
  axis(side = 2, at = ytcks1, labels = row_names, las = 2,
       cex.axis = axlabsize, tick = FALSE, line = lineadj)

  mtext(text = paste(MIP_name, " global land ", syear, "-", eyear, sep = ""),
