...
settings_plot <- yaml::read_yaml(args[1])
for (myname in names(settings_plot)) {
  temp <- get(myname, settings_plot)
  assign(myname, temp)
}

settings_input_file <- gsub("/work/", "/run/", settings_plot$input_files[1])
settings_input_file <- sub("/run/", "/work/", settings_input_file)

# list0_hlp <- yaml::read_yaml(settings$input_files[1])
settings_hlp <- list.files(path = settings_input_file, pattern = ".yml", all.files = FALSE,
           full.names = TRUE, recursive = FALSE,
           ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)

settings_ee <- yaml::read_yaml(settings_hlp)
list0 <- yaml::read_yaml(settings_ee$input_files[1])
...
# loop over variables
for (i in seq_along(settings_ee$input_files)) {
  metadata <- yaml::read_yaml(settings_ee$input_files[i])
...
work_dir <- settings_plot$work_dir
regridding_dir <- settings_plot$run_dir
plot_dir <- settings_plot$plot_dir
...
## Find prepared climdex indices in work folder
climdex_files_hlp <- list.files(path = settings_plot$input_files[1], pattern = "ETCCDI", full.names = TRUE)
file.copy(climdex_files_hlp, work_dir)
...
for (model_idx in c(1:length(models_name))) { # nolint
  ...
  if (!all(check_control)) {
    ...
    print(paste0("For ", models_name[model_idx]))
