...
gleckler_array <- function(path = land, idx_list = gleckler_idx,
  ...
  for (i in seq_along(idx_list)) {
    for (m in seq_along(model_list)) {
      ...
      tm_model <- nc_open(Sys.glob(file.path(path, paste0(
        "tm_", idx_list[i],
        "_", model_list[m],
        "*.nc"
      ))))
    ...
    ensarr <- array(NA, dim = c(
      nrow(tm_model_idx), ncol(tm_model_idx),
    ...
    for (j in 1:dim(ensarr)[3]) {
      ...
      for (o in seq_along(obs_list)) {
        tm_obs <- nc_open(Sys.glob(file.path(path, paste0(
          "tm_", idx_list[i],
          "_", obs_list[o],
          "*.nc"
        ))))
...
gleckler_plotting <- function(arr = rmserelarr, idx_list, model_list,
  ...
  par(xpd = TRUE)
  rect(
    xleft = 0, ybottom = 0, xright = (1 - 1 / xn), ytop = (1 - 1 / yn),
    density = NULL, angle = 45,
    col = NA, border = 1, lty = par("lty"), lwd = 4
  )
  rect(
    xleft = 0, ybottom = (1 - 1 / yn), xright = (1 - 1 / xn), ytop = 1,
    density = NULL, angle = 45,
    col = NA, border = 1, lty = par("lty"), lwd = 4
  )
  ...
  gleckler_scale <- function(sclseq, glc, xn, scaling_factor,
    ...
    for (a in 1:exlen) {
      if (a == 1) {
        ...
        polygon(x = xtmp, y = ytmp, col = glc[a])
        text(
          x = max(xtmp), y = max(ytmp), round(sclseq[a] + seq_shift, 1),
          cex = text.scaling_factor, pos = 4
        )
      }
    }
  ...
  for (zk in 1:nobs) {
    ...
    text(
      ...
      cex = gl_text_symb_scaling_factor, srt = rotx[zk]
    )
  }
  ...
  axis(
    side = 1, at = xtcks1, labels = col_names, las = 2,
  ...
  axis(
    side = 2, at = ytcks1, labels = row_names, las = 2,
    cex.axis = axlabsize, tick = FALSE, line = lineadj
  )
