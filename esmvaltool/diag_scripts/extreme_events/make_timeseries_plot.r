    ...
    for (idx in idx_list){
        ...
        if (chk.ts_data) {  
          ...
          returnvalue <- setTimeForFilesEqual(path = path, idx = idx,
                                              ...
                                              time_cropped = time_cropped) # This is a temporary solution
          ...
          for (m in modelAndObs) {
            ...
            regridAndLandSeaMask(idx_raw =paste0(time_cropped, "/", m),
                                 regrid = tsGrid,
...
time_series_preprocessing <- function(land = "./Land", idx = "tnnETCCDI_yr",
  ...
  pidx <- c("tn10pETCCDI_yr", "tx10pETCCDI_yr", "tn90pETCCDI_yr",
            "tx90pETCCDI_yr", "csdiETCCDI_yr", "wsdiETCCDI_yr",
            "tn10pETCCDI_mon", "tx10pETCCDI_mon", "tn90pETCCDI_mon",
            "tx90pETCCDI_mon", "csdiETCCDI_mon", "wsdiETCCDI_mon")
...
#    source('nml/cfg_ExtremeEvents/cfg_climdex.r')
    ...
    plotname <- paste(plot_dir, "/", idx,"_", length(obs_list),
                      "-obs_ensmean_timeseriesplot", sep = "")
    ...
    plot(time_conv, idx_ensm, type = "l", col = ts_col_list[n],
    ...
    for (o in obs_list) {
      ...
      lines(obsdata_list[[n]][[2]], obsdata_list[[n]][[3]],
            ...
            lwd=ts_lwd_list[n+1]) # plot observation
    }

    ## Produce a legend
    legend("top", legend = leg_names, col = ts_col_list, lty = ts_lty_list,
