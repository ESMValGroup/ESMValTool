...
def main(cfg, start_year, end_year):
    ...
    # Create a dictionary to store data for each variable_group
    data_by_variable_group = {}

    # Loop over variables/datasets
    for i, dataset in enumerate(input_data):
        ...
        # Check if variable_group is already in the dictionary
    ...
    # Get the minimum and maximum time values across all datasets
    min_time = min(data['cube'].coord('time').points.min() for data_list in data_by_variable_group.values() for data in data_list)
    max_time = max(data['cube'].coord('time').points.max() for data_list in data_by_variable_group.values() for data in data_list)
    ...
    for variable_group, data_list in data_by_variable_group.items():
        ...
        create_subplot(plt.gca(), data_list, variable_group, min_time, max_time)
