...
if (dim(reference_data)[3] != length(dates_historical)) {
    if (calendario == "365" | calendario == "365_days" |
        ...
        dates_historical <- dates_historical[-which(substr(dates_historical,
                            6, 10) == "02-29")]#nolint
    }
}

reference_data <- as.vector(reference_data)
dim(reference_data) <- c(model = 1, var = 1,
...
if (!is.na(mes)) {
    ...
    dims <- append(dims, c(length(ind) / length(years), length(years)),
                   after = time_dim)
} else if (!is.na(sea)) {
    reference_data <- SeasonSelect(reference_data, season = frequency, #nolint
    ...
    dims <- dim(reference_data)
    dims <- append(dims, c(length(time) / length(years), length(years)),
                   after = time_dim)
}
dims <- dims[-time_dim]
...
if (data_type == "day" | !is.na(sea)) {
    clim_obs <- aperm(apply(clim_obs,
      ...
      Loess, loess_span = 1), c(2, 3, 1, 4, 5))
}

anom_obs <- Ano(reference_data, clim_obs)
print(dim(anom_obs))
print(length(lon))
WR_obs <- WeatherRegime(data = anom_obs, EOFS = EOFS, lat = lat, #nolint
                        lon = lon, ncenters = ncenters,
...
if (region == "polar") {
    ...
    PlotLayout(PlotEquiMap, c(2, 3), lon = lon, lat = lat, #nolint
               var = cosa / 100, titles = title, filled.continents = FALSE,
               axelab = FALSE,  draw_separators = TRUE, subsampleg = 1,
               brks = seq(-1 * lim, lim, by = lim / 10), fileout = filepng1)
}
...
variable_list <- list(variable = WR_obs$composite, pvalue = WR_obs$pvalue,
                      cluster = WR_obs$cluster, frequency = WR_obs$frequency,
...
if (dim(projection_data)[3] != length(dates_projection)) {
    if (calendario == "365" | calendario == "365_days" |
        calendario == "365_day" | calendario == "noleap") {
        dates_projection <- dates_projection[-which(substr(dates_projection,
                                             6, 10) == "02-29")] #nolint
    }
}
...
if (!is.na(mes)) {
    ...
    projection_data <- SeasonSelect(projection_data, #nolint
                                  season = frequency,
...
if (data_type == "day" | !is.na(sea)) {
    clim_ref <- aperm(apply(clim_ref, c(1 : length(dim(clim_ref)))
...
if ((which(names(dim(reference)) == "lon") < #nolint
    ...
    which(names(dim(reference)) == "lat") &
    which(names(dim(anom_exp)) == "lon") <
    which(names(dim(anom_exp)) == "lat"))) { #nolint
...
WR_exp <- RegimesAssign(var_ano = anom_exp, ref_maps = reference, #nolint
                        lats = lat, method = "distance")
...
if (region == "polar") {
    filepng2 <- paste0(plot_dir, "/", frequency, "-", var0,
                       "_predicted_regimes.png")
    title <- paste0(paste0("Cluster ", 1 : dim(cosa)[1], " (",
             paste0("freq = ", round(WR_exp$frequency, 1), "%"), " )"))
    PlotLayout(PlotStereoMap, c(2, 3), lon = lon, lat = lat, #nolint
               var = cosa / 100, titles = title,
               filled.continents = FALSE, draw_separators = TRUE,
               subsampleg = 1, brks = seq(-1 * lim, lim, by = lim / 10),
               fileout = filepng2)
} else {
    filepng2 <- paste0(plot_dir, "/", frequency, "-", var0,
                       "_predicted_regimes.png")
...
variable_list <- list(variable = WR_exp$composite, pvalue = WR_exp$pvalue,
                      cluster = WR_exp$cluster, frequency = WR_exp$frequency,
                      lat = lat, lon = lon, time = time)
names(variable_list)[1] <- var0

attributes(variable_list) <- NULL
filencdf2 <-   paste0(work_dir, "/", var0, "_", frequency, "_WR_exp_",
                      model_names, "_", start_projection, "_", end_projection,
                      "_", start_historical, "_", end_historical, ".nc")
...
if ((which(names(dim(reference)) == "lon") < #nolint
    ...
    which(names(dim(reference)) == "lat") &
    which(names(dim(cosa)) == "lon") <
    which(names(dim(cosa)) == "lat"))) { #nolint
        dim_names <- names(dim(reference))
        pos_lon <- which(names(dim(reference)) == "lon")
        pos_lat <- which(names(dim(reference)) == "lat")
        pos <- 1 : length(dim(reference))
...
filepng3 <- paste0(file.path(plot_dir, "Table_"), var0, "_", frequency,
                   ...
                   end_projection, "_", start_historical,
