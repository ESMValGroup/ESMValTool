...
month_names <- c("JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AGO", "SEP", "OCT", "NOV", "DEC")
if(moninf == monsup) {
...
# Provisional solution to error in dimension order and time values:
 ...
 if ((end_climatology-start_climatology + 1) * 12 == length(time)) {
     time <-  seq(as.Date(paste(start_climatology, '01', '01', sep = "-"), format = "%Y-%m-%d"), as.Date(paste(end_climatology, '12', '01', sep = "-"), format = "%Y-%m-%d"), "month")
 }
...
if (moninf <= monsup) {
   ...
   if (monsup == 2 & moninf == 12) {
       reference_seasonal_mean <- SeasonSelect(reference_data, season = 'DJF', dates = time, calendar = calendar)$data
...
if ((end_anomaly-start_anomaly + 1) * 12 == length(time)) {
    time <-  seq(as.Date(paste(start_anomaly, '01', '01', sep = "-"), format = "%Y-%m-%d"), as.Date(paste(end_anomaly, '12', '01', sep = "-"), format = "%Y-%m-%d"), "month")
}
...
if (moninf <= monsup) {
    ...
    rcp_seasonal_mean <- Season(rcp_data, posdim = time_dim, monini = monini, moninf = moninf,
                                   monsup = monsup)
...
climatology <- InsertDim(climatology, years_dim, lendim = dim(rcp_seasonal_mean)[years_dim])
...
model_anomalies <- WeightedMean(anomaly, lon = as.vector(lon), lat = as.vector(lat), mask = NULL)
 if (!is.null(params$running_mean)) {
  model_anomalies <- Smoothing(model_anomalies, runmeanlen = params$running_mean, numdimt = 2)
}
...
file <- nc_create(paste0(plot_dir, "/", var0, "_",months, "_multimodel-anomaly_",
              model_names_filename,"_", start_anomaly, "_", end_anomaly,"_", start_climatology, "_", end_climatology, ".nc"), list(defdata, defagreement))
