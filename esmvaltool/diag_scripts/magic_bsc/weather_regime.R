...
Loess <- function(clim, loess_span) {
  if (sum(is.na(clim)) != length(clim)) {
    data <- data.frame(ensmean = clim, day = seq_len(length(clim)))
...
if (data_type == "day" | !is.na(sea)) {
  clim_ref <- aperm(apply(clim_ref, c(seq_len(length(dim(
    clim_ref
  ))))
...
if ((
  ...
  pos_lon <- which(names(dim(reference)) == "lon")
  pos_lat <- which(names(dim(reference)) == "lat")
  pos <- seq_len(length(dim(reference)))
  pos[pos_lon] <- pos_lat
  pos[pos_lat] <- pos_lon
