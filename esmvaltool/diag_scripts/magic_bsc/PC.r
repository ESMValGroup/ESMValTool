...
{	#----------------
  ...
  #----------------
  attr <- strsplit(trimws(system(paste("perl -e 'open FH,\"",file,"\";while(<FH>){@parts= /^# (.+): (.+) /;print \"@parts \";}'",sep=""),intern=T)),"\\s+")
...
{	#----------------
  ...
  {	# get pc for standard air density
    ...
    pc$points <- transform(pc$points,WindSpeed=as.numeric(as.character(WindSpeed)),Power=as.numeric(as.character(Power)))
  ...
  # Create an approximating function
  #----------------
  pc$fun <- approxfun(pc$points$WindSpeed,pc$points$Power,method="linear",yleft=NA,yright=0,ties="ordered")
