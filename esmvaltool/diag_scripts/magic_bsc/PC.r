...
read_pc <- function(file) {
  ...
  pc$fun <- approxfun(pc$points$WindSpeed, pc$points$Power, # nolint
                    method = "linear",
                    yleft = NA, yright = 0)
...
read_xml_pc <- function(file) {
  ...
  for (i in 1 : length(pcs)) {
    ...
    pc$points <- transform(
      pc$points,
      WindSpeed = as.numeric(as.character(WindSpeed)), # nolint
  ...
  pc$fun <- approxfun(
    pc$points$WindSpeed, #nolint
    pc$points$Power, # nolint
    method = "linear",
    yleft = NA,
    yright = 0,
    ties = "ordered"
  )
  pc$attr$Diameter <- xml_data$wind_turbine_properties$rotor_diameter # nolint
  pc$attr$CutIn <- NA # nolint
  pc$attr$CutOut <- NA # nolint
  pc$attr$ReCutIn <- NA # nolint
  pc$attr$RatedSpeed <- NA # nolint
  pc$attr$RatedPower <- xml_data$wind_turbine_properties$rated_power # nolint
...
plot_pc_list <- function(list_pcs) {
  list_funs <- lapply(list_pcs, function(x) {
      function(y) {
            x$fun(y) / x$attr$RatedPower} # nolint
  })
  names <- lapply(list_pcs, function(x) x$attr$Name )  # nolint
  plot <- ggplot(NULL, aes(x = x, colour = Turbine)) #nolint
  for (i in 1 : length(list_pcs)) {
    plot <- plot + stat_function(data = data.frame(x = 0 : 30,
            Turbine = factor(names[[i]])), fun = list_funs[[i]])
  }
  plot <- plot + xlab("Wind speed (m/s)") + ylab("Capacity Factor (%)") +
