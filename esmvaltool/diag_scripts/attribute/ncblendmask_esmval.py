...
def ncblendmask_esmval(
    ...
    sftof = 1 - numpy.ma.filled(
        nc.variables["sftlf"][:, :], -1.0e30
    )  # NPG - added '1-' to use lf.
    nc.close()
    ...
    if "m" in options:
        ...
        if tas.shape[0] > obs_tas.shape[0]:
            obs_tas = numpy.concatenate(
                (
                    obs_tas,
                    numpy.full(
                        (
                            tas.shape[0] - obs_tas.shape[0],
                            obs_tas.shape[1],
                            obs_tas.shape[2],
                        ),
                        fill_value=-1e30,
        ...
        regrid_index = list(
            range(int(lons5.shape[0] * 0.5), lons5.shape[0])
        ) + list(range(int(lons5.shape[0] * 0.5)))
    ...
    wm = w.copy()
    if "m" in options:
        wm[cvgmsk[0 : wm.shape[0], :, :] < -100] = 0.0
    # calculate diagnostic
    diag = calc_diag(
        tos, wm, diag_name
    )  # Diagnostic for attribution analysis.
    if gmst_flag:
        dec_warming.append(
            calc_dec_warming(tos, w, warming_years, sr15_flag)
        )  # Diagnose GMST warming with global coverage for attributable trends.
    else:
        dec_warming.append(
            calc_dec_warming(tas, w, warming_years, sr15_flag)
        )  # Diagnose SAT warming with global coverage for attributable trends.
    obs_dec_warming.append(
        calc_dec_warming(obs_tas, wm, warming_years, sr15_flag)
    )

    if ann_warming != 0:
        ann_warming.append(calc_ann_warming(tas, w))  # Calculate ann warming.
    if gmst_comp_warming != 0:
        gmst_comp_warming.append(
            calc_ann_warming(tos, w)
    ...
    if numpy.amax(diag) > 10:
        print("Error: Diagnostic to high, stopping")
        exit()
    ...
    if ensobs != "":
        ...
        for ens in range(1, enssize + 1):
            ...
            obs_tas = obs_tas[
                :, :, regrid_index
            ]  # Regrid to match esmvaltool output.
            ensobs_dec_warming.append(
                calc_dec_warming(
                    obs_tas[0 : tas.shape[0], :, :],
                    wm,
                    warming_years,
                    sr15_flag,
                )
            ...
            ensobs_diag.append(
                calc_diag(obs_tas[0 : tas.shape[0], :, :], wm, diag_name)
            )
...
def calc_diag(tos, wm, diag_name):
    ...
    nper = math.ceil(
        tos.shape[0] / av_per
    )  # Round up number of averaging periods.
    ...
    diag = diag - numpy.nanmean(
        diag, axis=1, keepdims=True
    )  # Take anomalies over whole period.
...
def calc_dec_warming(tas, w, warming_years, sr15_flag):
    ...
    if sr15_flag:
        data = gmt_mon[
            (warming_years[0] - 1850) * 12 : (warming_years[1] - 1849) * 12
        ] - numpy.mean(gmt_mon[0 : (1901 - 1850) * 12])
        data = numpy.mean(
            data.reshape(warming_years[1] - warming_years[0] + 1, 12), axis=1
        )
        years = list(
            range(warming_years[0] - warming_years[1], 1, 1)
        )  # Use sequence of years ending at 0 so intercept is at last year.
        print("years:", years)
        slope, warming = numpy.polyfit(years, data, deg=1)
    else:
        warming = numpy.nanmean(
            gmt_mon[
                (warming_years[0] - 1850) * 12 : (warming_years[1] - 1849) * 12
            ]
        ) - numpy.mean(gmt_mon[0 : (1901 - 1850) * 12])
...
def calc_ann_warming(tas, w):
    ...
    for m in range(nyr):
        diag[m] = numpy.mean(
            gsat_mon[m * 12 : (m + 1) * 12]
        )  # Note - will calculate average over incomplete final year.
    diag = diag - numpy.mean(
        diag[0 : (1901 - 1850)]
    )  # Take anomalies relative to 1850-1901.
    return diag
