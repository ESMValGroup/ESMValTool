...
set_time_for_files_equal <- function(path, # nolint
  ...
  for (i in seq_along(model_list)) {
      pattern_splitted <- strsplit(model_list[i],"_")[[1]][1:5] 
      pattern_remerged <- paste(pattern_splitted, collapse='_') 
      models[i] <- models_avail[grep(
      pattern = pattern_remerged, 
      x = models_avail
    )]
  }
  ...
  for (m in models) {
    ...
    # solving the timeframe slicing problem (by FR)
    year_block <-  strsplit(m, "_")[[1]][8]
    se_year <- strsplit(year_block, ".nc")[[1]][1]
    s <- strsplit(se_year,"-")[[1]][1]
    e <- strsplit(se_year,"-")[[1]][2]
    start[i] <- strtoi(substr(s, 1,4))
    end[i] <- strtoi(substr(e, 1,4))
