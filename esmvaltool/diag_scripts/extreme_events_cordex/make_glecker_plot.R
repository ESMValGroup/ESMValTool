...
gleckler_main <-
  function(path = "../work/extreme_events",
             idx_list,
             model_list,
             obs_list,
             plot_dir = "../plot",
...
gleckler_array <- function(path = land,
  ...
  for (i in seq_along(idx_list)) {
    for (m in seq_along(model_list)) {
      ...
      if (lat_collect) {
		all_coords_list <- attributes(tm_model$dim)$names # all dimensions names
		# find the string corresponding to latitude ("lat" or "rlat")
		lat_str <- all_coords_list[grepl("lat", all_coords_list)]
        model_lat <- ncvar_get(tm_model, lat_str)
        lat_collect <- FALSE
      }
...
gleckler_plotting <-
  function(arr = rmserelarr,
    ...
    ## Add model labels
	### create a short name for each model 
	splitted_names <- strsplit(model_list, split = "_")
	# drop the version
	for (i in seq_along(splitted_names)){
		splitted_names[[i]] = splitted_names[[i]][1:4]
	}
	short_names <- unlist(
					lapply( splitted_names, 
							function(x) {
								y=substr(x,1,3)
								paste(y, collapse="_")
							}
    ...
    col_names <- c("ENSMEAN", "ENSMEDIAN", short_names)
