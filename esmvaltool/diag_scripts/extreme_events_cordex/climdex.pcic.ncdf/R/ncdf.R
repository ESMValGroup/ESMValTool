...
get.climdex.variable.metadata <- function(vars.list, template.filename) {
  ...
  # CMIP5 convenction doesn't work properly (by FR)
  ...
  cmor_name <- gsub("var_timeres", "", template.filename)
  fnames <- lapply(rownames(all.data), paste0,cmor_name)
  all.data$filename <- fnames
