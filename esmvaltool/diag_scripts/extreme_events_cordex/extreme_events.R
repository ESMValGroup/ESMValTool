...
#          Francesco Repola (CMCC, Italy)
# #############################################################################
# Description
#    Calculate extreme events with plotting functionality
#
# Modification history
#    20210526-repola_francesco: adaptation to regional models
...
# import local scripts and configurations
diag_scripts_dir <- Sys.getenv("diag_scripts")
climdex_src <-
  paste0(
    diag_scripts_dir,
    "/extreme_events_cordex/climdex.pcic.ncdf/R/ncdf.R"
  ) # nolint
source(paste0(
  diag_scripts_dir,
  "/extreme_events_cordex/climdex.pcic.ncdf/R/ncdf.R"
...
source(paste0(diag_scripts_dir, "/extreme_events_cordex/cfg_climdex.R")) # nolint
source(paste0(diag_scripts_dir, "/extreme_events_cordex/cfg_extreme.R")) # nolint
source(
  paste0(
    diag_scripts_dir,
    "/extreme_events_cordex/common_climdex_preprocessing_for_plots.R"
  )
) # nolint
source(paste0(
  diag_scripts_dir,
  "/extreme_events_cordex/make_timeseries_plot.R"
)) # nolint
source(paste0(
  diag_scripts_dir,
  "/extreme_events_cordex/make_glecker_plot.R"
)) # nolint
...
# extract metadata
# models_name <- unname(sapply(list0, "[[", "dataset")) # by FR 
...
for (i in seq_along(settings$input_files)) {
  ...
  models_rcm <- unname(sapply(metadata, "[[", "dataset")) # by FR
  short_name <- unname(sapply(metadata, "[[", "short_name"))
  models_driver <- unname(sapply(metadata, "[[", "driver")) # by FR 
  models_ensemble <- unname(sapply(metadata, "[[", "ensemble")) # by FR
  models_experiment <- unname(sapply(metadata, "[[", "exp")) # by FR
  models_version <- unname(sapply(metadata, "[[", "rcm_version")) # by FR
  variables <- c(variables, short_name)
  models_name <- c(models_name, paste0(models_driver, "_", models_rcm)) # by FR 
  mdl <- paste0(models_driver, "_", models_rcm, "_", 
                models_experiment, "_", models_ensemble, "_", models_version) # by FR
  models <- c(models, mdl) # by FR 
...
##
for (model_idx in c(1:length(models))) { # nolint
  ...
  template <- paste(
    ...
    "_",
    models_version[model_idx], # by FR
    "_",            # by FR 
  ...
  if (!all(check_control)) {
    print("")
    print(paste0(">>>>>>> Producing Indices for ", models[model_idx]))
    print("")
    # infiles <- climofiles[models == models_name[model_idx]] # by FR
    infiles <- climofiles[models == models[model_idx]] # by FR 
    ...
    infiles <- climofiles[models == models[model_idx]]
...
write_plots <- if (ts_plt || glc_plt) TRUE else FALSE
if (write_plots) { # nolint
  ...
  use_obs <- if (is.null(reference_datasets)) FALSE else TRUE # observations are an optional FR
  ###################################
  ...
  print(if(use_obs) setdiff(models, reference_datasets) else models) # by FR
  ...
  if (ts_plt) {
    ...
    mdl <- if (use_obs) setdiff(models, reference_datasets) else models # by FR 
    ...
    ancestors <- sapply( if(use_obs) setdiff(models, reference_datasets) else models, # by FR
      grep,
      climofiles,
      value = TRUE
    )
  ...
  if (glc_plt) {
    ...
    nmodel <- length(models) # number of models
    ...
    plotfiles <- gleckler_main(
      ...
      model_list = setdiff(models, reference_datasets),
      obs_list = reference_datasets,
      plot_dir = plot_dir,
