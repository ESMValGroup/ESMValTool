...
#    20210528-repola_francesco:      observations are an optional
...
timeseries_main <- function(path = "../work/extreme_events",
  ...
  use_obs <- if (is.null(obs_list)) FALSE else TRUE # observations are an optional FR
  ...
  for (idx in idx_list) {
    ...
    modelobs_list <- if (use_obs) unique(c(model_list, obs_list)) else unique(model_list) # FR 
...
timeseries_plot <-
  function(plot_dir = "./plot",
    ...
    # Drawing parameters
    use_obs <- if (is.null(obs_list)) FALSE else TRUE # observations are an optional FR
    leg_names <- if(use_obs) c(mip_name, obs_list) else mip_name # by FR 
    ...
    temporary_string <- if(use_obs) "-obs_ensmean_timeseriesplot" else "-ensmean_timeseriesplot" # by FR
    plotname <- paste(plot_dir,
      "/",
      idx,
      "_",
      length(obs_list),
      temporary_string, # by FR
      sep = ""
    )
