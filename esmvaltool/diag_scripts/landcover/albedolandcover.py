...
# A switch, that could be added to a configuration file
fall_into_ipython = True
...
def main(cfg):
    ...
    def _add_masks_albedolandcover(model_data):
    ...
    def _get_reconstructed_albedos(model_data):
        ...
        for i, j in it.product(range(model_data['alb'].shape[0]),
            ...
            if model_data['alb'].data[i, j]:  # if it is not masked
                ...
                if nvalid_bbox > dia_cfg['minnum_gc_bb']:
                    ...
                    lc_classes = [lc1_class, lc2_class, lc3_class]
                    lc_data = []
    ...
    def _write_albedochanges_to_disk(alb_lc):
        result_dict = {'lc1': alb_lc[0, :, :], 'lc2': alb_lc[1, :, :],
                       'lc3': alb_lc[2, :, :]}
        names = {'lc1': '-'.join(lc1_class), 'lc2': '-'.join(lc2_class),
                 'lc3': '-'.join(lc3_class)}
        for ikey, jkey in it.product(result_dict.keys(), result_dict.keys()):
            if not ikey == jkey:
                ...
                transition_cube = model_data['snc'].copy(transition_data)
                transition_cube.rename(transition_name)
                # Get some usefull info for constructing the filenames
                modelname = datadict['alb']['dataset']
                template_time = model_data['alb'].coord('time')
                month_string = template_time.units.num2date(
                    template_time.points)[0].strftime('%b')
                basename = '{0}-{1}-{2}'.format(month_string, modelname,
                                                transition_name)
    ...
    for dataset_name in my_files_dict:
        ...
        this_models_xxfracs = [key for key in datadict.keys() if 'Frac' in key]
        lc3_class = [key for key in this_models_xxfracs if key in lc3_class]
        ...
        alb_lc = _get_reconstructed_albedos(model_data)

        # Calculate differences between them
        _write_albedochanges_to_disk(alb_lc)
