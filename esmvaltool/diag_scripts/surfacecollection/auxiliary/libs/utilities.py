...
def __unify_meta__(m1, m2):
    """
    combines 2 metadata strings if possible
    ---------------------------------------
...
def set_metadata(c1, c2, fun):
    ...
    sta = __unify_meta__(c1m.standard_name, c2m.standard_name)

    lon = __unify_meta__(c1m.long_name, c2m.long_name)

    var = __unify_meta__(c1m.var_name, c2m.var_name)

    uni = __unify_meta__(c1m.units, c2m.units)

    att = __unify_attributes__(c1m.attributes, c2m.attributes)
    cel = fun

    meta = iris.cube.CubeMetadata(
        standard_name=sta,
        long_name=lon,
        var_name=var,
        units=uni,
    ...
    if len(list(set([len(t.points) for t in tcoords]))) == 1:
        ...
        for loc in list_of_cubes:
            try:
                ...
                # preprocessor should make sure that 0 is the right coordinate
...
def weighted_spatial_mean(cube):
    ...
    for latlon in cube.slices(["latitude", "longitude"]):
        time_mean = latlon.collapsed(["latitude", "longitude"],
                                     iris.analysis.MEAN,
                                     weights=spw)

        cubes.append(time_mean)
...
def __check_temporal_basis__(kwargs):
    """
    checks the temporal basis in kwargs
    -----------------------------------------
    returns temporal basis
    """
    if "temporal_basis" not in kwargs.keys():
        logger.error("option temporal_basis required for this diagnostic")
        raise ValueError("recipe is missing the temporal_basis keyword")
    else:
...
class InputHandler(object):
    ...
    def __init__(self, **kwargs):
        ...
        super(InputHandler, self).__init__(**kwargs)
    ...
    def apply_iris_fun(self, fun, ctype, **kwargs):
        ...
        if ctype == "method":
            ...
            elif not isinstance(kwargs["other"], InputHandler):
