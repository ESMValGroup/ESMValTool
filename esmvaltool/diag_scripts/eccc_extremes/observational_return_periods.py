...
def obtain_covariate_data(
    ...
    """
    all_covariates = select_metadata(
        input_data.values(), variable_group="obs_covariate"
    )
    if all_covariates:
        covariate_meta = select_metadata(all_covariates, dataset=dataset)
        if not covariate_meta:
            covariate_meta = select_metadata(
                all_covariates, dataset=covariate_dataset
            )
            if not covariate_meta:
                raise ValueError(
                    f"Since the dataset {dataset} does not "
        ...
        if smooth_kernel:
            covariate_data = covariate_data.rolling(
                time=smooth_kernel, min_periods=1
            ).mean()
        if add_year:
            last_value = covariate_data[-1]
            last_date = datetime.strptime(
                str(last_value.time.values).split(".")[0], "%Y-%m-%dT%H:%M:%S"
            )
            ...
            covariate_data = xr.concat(
                [covariate_data, last_value.assign_coords(time=new_time)],
                dim="time",
            )
    else:
        covariate_data = None
...
def calculate_anomaly(
    data: xr.DataArray, anomaly_dic: dict, name: str, input_data: dict
):
    """
    Parameters
    ----------
    ...
    base_year_end = base_period[1]
    base_data = data.sel(
        time=slice(f"{base_year_start}-01-01", f"{base_year_end}-12-31")
    )
    if len(base_data) == 0:
        logger.info(
            f"The base period {base_year_start}-{base_year_end} "
            f"wasn't available in the data file for dataset {name}. "
            "Looking for an 'anomaly' group for this dataset."
        )
        base_meta = select_metadata(
            input_data.values(), dataset=name, variable_group="anomaly"
        )
        if not base_meta:
            raise ValueError(
                f"No data for {name} in 'anomaly' group has been found. "
            )
        else:
            ...
            if base_data.name != base_short_name:
                raise ValueError(
                    f"The short name ({base_short_name}) of the "
...
class StationaryRP:
    ...
    def obtain_pdf(self, x_gev: np.ndarray | None = None):
        ...
        self.pdf = gev.pdf(
            self.x_gev, self.shape, loc=self.loc, scale=self.scale
        )
...
class NonStationaryRP:
    ...
    def __init__(
        ...
        if len(data) != len(covariate):
            raise ValueError(
                "The lengths of the data and covariatedon't match."
            )
        stat_gev = cex.fit_gev(
...
def obtain_confidence_intervals(
    data: xr.DataArray,
    event: float,
    conf_int: list[int | float],
    seed: int = 1,
    yblock: int = 1,
    ...
    if len(data) % yblock > 0:
        logger.warning(
            f"The data length {len(data)} is not devisible by "
    ...
    for _i in range(1000):
        ...
        if covariate_data is None:
            ...
            TmpNonStat = NonStationaryRP(
                data[fit_idx].data, covariate_data[fit_idx].data, event
            )
    ...
    cis = np.nanpercentile(rps, conf_int, method="closest_observation").round(
        1
    )
    out_dic = {}
...
class SingleObsDataset:
    ...
    def __init__(self, dataset: str, path: str, cfg: dict):
        ...
        if cfg.get("anomaly"):
            data = calculate_anomaly(
                data, cfg["anomaly"], dataset, all_input_data
            )
        ...
        covariate_data = obtain_covariate_data(
            all_input_data, dataset, covariate_dataset, smooth_kernel, add_year
        )
        ...
        if self.covariate_data is None:
            logger.info(
                f"For {dataset} only the stationary GEV was calculated"
            )
        else:
            self.NonStationaryRP = NonStationaryRP(
                self.data.data, self.covariate_data.data, self.event, idx
            )
            self.non_stationary_cis = obtain_confidence_intervals(
                ...
                covariate_value=self.NonStationaryRP.cov_value,
                yblock=cfg.get("yblock"),
            )
    ...
    def reform_to_yml_dic(self):
        ...
        dataset_dic["event"] = self.event
        dataset_dic["year"] = self.year
        dataset_dic["units"] = self.units
        dataset_dic["stationary_gev"] = {
            "shape": self.StationaryRP.shape,
...
def plot_figure(Dataset: SingleObsDataset, cfg: dict, prov_dic: dict):
    ...
    """
    prov_dic["caption"] = (
        f"{Dataset.name} GEV statistics for" + f" the {Dataset.year} event"
    )
    ...
    min_val = np.floor(
        Dataset.data.min() * 0.9
        if Dataset.data.min() > 0
        else Dataset.data.min() * 1.1
    )
    max_val = np.ceil(
        Dataset.data.max() * 1.1
        if Dataset.data.max() > 0
    ...
    if Dataset.covariate_data is None:
        obs_surv_func = gev.sf(
            x_fine,
            Dataset.StationaryRP.shape,
    ...
    fig_obs.set_size_inches(12.0, 8.0)
    ...
    ax_hist.hist(
        Dataset.data.data,
        bins=bins,
        edgecolor=color_st["facecolor"],
        facecolor=color_st["color"],
        alpha=0.3,
        density=True,
    )
    ax_hist.scatter(
    ...
    ax_surv.plot(
        x_fine[obs_surv_func > 0.00001],
        1 / obs_surv_func[obs_surv_func > 0.00001],
        c=color_st["color"],
    )
    ax_surv.scatter(
        Dataset.event,
    ...
    ax_surv.text(
        0.15,
        0.5,
        f"{Dataset.name} return period\n{rp} years",
        transform=ax_surv.transAxes,
        clip_on=False,
    )

    ax_tseries.plot(
        Dataset.data.year.data, Dataset.data.data, c=color_st["color"]
    )
    ax_tseries.scatter(
        Dataset.year,
        Dataset.event,
        ...
        s=100,
        facecolors="None",
        edgecolors=color_st["facecolor"],
    )
    ax_tseries.set_ylim(bins[0], bins[-1])
    ax_tseries.set_xlim(
        Dataset.data.year.data[0] - 0.5, Dataset.data.year.data[-1] + 0.5
    )
    ax_tseries.text(
        0.7,
        0.92,
        f"{Dataset.name}({Dataset.year}): "
        + str(np.around(Dataset.event, 1))
        + f" {units}",
        transform=ax_tseries.transAxes,
        clip_on=False,
    )
    ...
    fig_path = os.path.join(
        cfg["plot_dir"], f"{Dataset.name}_{reg}_{label.lower()}"
    )
...
def main(cfg):
    ...
    obs_data_metadata = select_metadata(
        cfg["input_data"].values(), variable_group="obs_data"
    )
