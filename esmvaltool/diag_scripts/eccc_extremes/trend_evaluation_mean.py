...
def obtain_reference(data_group: list):
    ...
    for dataset_f in ref_fnames.keys():
        ...
        reference_dic[key] = {}
        reference_dic[key]['trend'] = eprep.linear_trend(ref_cb).data
        reference_dic[key]['std_err'] = eprep.linear_trend_stderr(ref_cb).data
...
def plot_means(data_dic, reference_dic, cfg):
    ...
    for ref_dataset in reference_dic.keys():
        ...
        ref_line = ax_means.axvline(reference_dic[ref_dataset]['trend'], c=ref_color_st['color'], zorder=2, 
                                                        label=ref_dataset)
        ax_means.fill_betweenx([len(data_dic.keys()) -0.8, -0.2], 
                                 reference_dic[ref_dataset]['trend'] - reference_dic[ref_dataset]['std_err'],
                                 reference_dic[ref_dataset]['trend'] + reference_dic[ref_dataset]['std_err'],
    ...
    ax_means.set_xlabel(f'Mean of {exp_variable}, {units}')

    default_caption = f'{variable} trend variability in {region}'
    ...
    fig_path = os.path.join(cfg['plot_dir'], f'trend_variability_{variable}_{region}')
