"""
Diagnostic to reproduce fig. 6 from Malinina&Gillett (2024)

This diagnostic calculates a standard deviation for individual model
realizations and if requested weighted by the number of realizations
standard deviation for all the models. The resulting plot shows
indivisual model realizations as dots and all models as the square.
The reference dataset is shown as the vertical line.

The reqired variable groups are `reference` and other data variable
group. The preprocessed data should be a 1D cube.

The mandatory keywords for diagnostic are required:
    `model_weighting`: boolean flag showing if multi-model std_dev
                       should be weighted
...
def calculate_ensemble_stdev(
    ens_cubelist: iris.cube.CubeList, weight_flag: bool
):
    ...
    for i in range(0, len(ens_cubelist)):
        cb = ens_cubelist[i]
        cb_wght = (
            cb.attributes["ensemble_weight"] * cb.attributes["reverse_dtsts_n"]
        )
...
def plot_stdevs(data_dic, reference_dic, cfg):
    ...
    fig_stds.set_size_inches(8.0, 9.0)
    ...
    for ref_dataset in reference_dic.keys():
        ref_color_st = eplot.get_dataset_style(
            ref_dataset, cfg.get("color_style")
        )
        ref_line = ax_stds.axvline(
            reference_dic[ref_dataset],
    ...
    caption = (
        cfg["figure_caption"] if cfg.get("figure_caption") else default_caption
    )
    ...
    fig_path = os.path.join(
        cfg["plot_dir"], f"variability_{variable}_{region}"
    )
...
def main(cfg):
    ...
    multi_model_std = calculate_ensemble_stdev(
        ens_var_cubelist, cfg.get("model_weighting")
    )
