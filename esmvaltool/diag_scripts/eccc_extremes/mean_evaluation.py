...
def obtain_reference(data_group: list):
    ...
    for dataset_f in ref_fnames.keys():
        ...
        reference_dic[key] = {}
        reference_dic[key]['mean'] = ref_cb.data.mean()
        reference_dic[key]['std'] = ref_cb.data.std()
...
def plot_means(data_dic, reference_dic, cfg):
    ...
    for ref_dataset in reference_dic.keys():
        ...
        ref_line = ax_means.axvline(reference_dic[ref_dataset]['mean'], c=ref_color_st['color'], zorder=2, 
                                                        label=ref_dataset)
        ax_means.fill_betweenx([len(data_dic.keys()) -0.8, -0.2], 
                                 reference_dic[ref_dataset]['mean'] - 3*reference_dic[ref_dataset]['std'],
                                 reference_dic[ref_dataset]['mean'] + 3*reference_dic[ref_dataset]['std'],
