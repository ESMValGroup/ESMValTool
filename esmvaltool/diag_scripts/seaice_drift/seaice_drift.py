...
class SeaIceDrift():
    ...
    def compute(self):
        ...
        for filename in sithick_files:
            reference_dataset = self._get_reference_dataset(
                'sithick', self.datasets.get_info('reference_dataset',
                                                  filename))
            alias = self._get_alias(filename, reference_dataset)
        ...
        for filename in sispeed_files:
            reference_dataset = self._get_reference_dataset(
                'sispeed', self.datasets.get_info('reference_dataset',
                                                  filename))
            alias = self._get_alias(filename, reference_dataset)
            if obs_file and alias == 'reference':
                ...
                sispeed = iris.cube.Cube(obs_data,
                                         'sea_ice_speed',
                                         units='km day-1')
                sispeed.add_dim_coord(
                    iris.coords.DimCoord(range(1, 13),
                                         var_name='month_number'), 0)
                sispeed.add_dim_coord(
                    iris.coords.DimCoord(range(1979, 1979 + 35),
                                         var_name='year'), 1)
                sispeed.extract(
    ...
    def _get_mask(self, data, filename):
        ...
        var_info = esmvaltool.diag_scripts.shared.group_metadata(
            self.cfg['input_data'].values(), 'alias')[dataset_info[n.ALIAS]]
        var_info = esmvaltool.diag_scripts.shared.group_metadata(
            var_info, 'short_name')
    ...
    def _get_slope_ratio(siconc, drift):
        ...
        std_dev, sig = SeaIceDrift._sd_slope(slope, intercept, siconc.data,
                                             drift.data)
    ...
    def _print_results(self, model):
        ...
        if 'latitude_treshold' in self.cfg:
            logger.info('Metrics computed over domain north of %s',
                        self.cfg['latitude_treshold'])
        else:
            logger.info('Metrics computed inside %s region',
    ...
    def _save_slope(self, dataset):
        ...
        self._create_prov_record(siconc_path, caption,
        ...
        self._create_prov_record(sivol_path, caption,
    ...
    def _plot_domain(self, dataset):
        ...
        caption = ("Drift - sithick and drift - siconc plot between "
    ...
    def _plot_drift_sivol(self, axes, dataset):
        ...
        axes.plot([sivol[-1], sivol[0]], [drift[-1], drift[0]],
                  'r-',
                  linewidth=2)
    ...
    def _get_data_and_ancestors(self, dataset, var):
        ...
        data = esmvaltool.diag_scripts.shared.group_metadata(
            self.cfg['input_data'].values(), 'alias')[dataset]
        data = esmvaltool.diag_scripts.shared.group_metadata(
            data, 'short_name')
        return (data[var][0],
...
class InsidePolygonFactory(AuxCoordFactory):
    ...
    def make_coord(self, coord_dims_func):
        ...
        * coord_dims_func:
        ...
        in_polygon = iris.coords.AuxCoord(points,
