...
def _get_and_plot_multimodel(cfg, cube, all_drought, input_filenames):
    ...
    all_drought_mean["Difference"] = (
        (all_drought_mean["Future"] - all_drought_mean["Historic"])
        / (all_drought_mean["Future"] + all_drought_mean["Historic"])
        * 200
    )
...
def _set_tscube(cfg, cube, time, tstype):
    ...
    if tstype == "Future":
        ...
        start = datetime.datetime(
            cfg["end_year"] - cfg["comparison_period"], 1, 15, 0, 0, 0
        )
        ...
        start = datetime.datetime(cfg["start_year"], 1, 15, 0, 0, 0)
            ...
            cfg["start_year"] + cfg["comparison_period"], 12, 16, 0, 0, 0
        )
...
def main(cfg):
    ...
    for iii, spei_file in enumerate(glob.iglob(input_filenames)):
        ...
        if first_run == 1:
            ncfiles = list(
                filter(
                    lambda f: f.endswith(".nc"),
                    os.listdir((cfg[n.INPUT_FILES])[0]),
                )
            )
            all_drought = {}
            all_drought["Historic"] = np.full(
                cube2.data.shape + (4,) + (len(ncfiles),), np.nan
            )
            all_drought["Future"] = np.full(
                cube2.data.shape + (4,) + (len(ncfiles),), np.nan
            )
            first_run = 0
        # Test if time series goes until cfg['end_year']/12
        timecheck = time.units.date2num(
            datetime.datetime(cfg["end_year"], 11, 30, 0, 0, 0)
        )
    ...
    _get_and_plot_multimodel(
        cfg, cube, all_drought, glob.glob(input_filenames)
    )
