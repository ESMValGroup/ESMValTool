...
def _get_and_plot_obsmodel(
    cfg, cube, all_drought, all_drought_obs, input_filenames
):
    ...
    perc_diff = (
        (all_drought_obs - all_drought_hist_mean)
        / (all_drought_obs + all_drought_hist_mean)
        * 200
    )
    ...
    _plot_multi_model_maps(
        cfg, all_drought_hist_mean, [lats, lons], input_filenames, "Historic"
    )
    _plot_multi_model_maps(
        cfg, all_drought_obs, [lats, lons], input_filenames, "Observations"
    )
    _plot_multi_model_maps(
        cfg, perc_diff, [lats, lons], input_filenames, "Difference"
    )
...
def ini_time_series_plot(cfg, cube, area, filename):
    ...
    ).collapsed(coords, iris.analysis.MEAN, weights=cube_grid_areas)
...
def main(cfg):
    ...
    input_filenames = (
        (cfg[n.INPUT_FILES])[0] + "/*_" + (cfg["indexname"]).lower() + "_*.nc"
    )
    print(cfg.keys())
    first_run = 1
    iobs = 0
    ...
    for iii, spei_file in enumerate(glob.iglob(input_filenames)):
        ...
        if first_run == 1:
            ...
            shape_all = cube2.data.shape + (4,) + (len(ncfiles) - 1,)
    ...
    _get_and_plot_obsmodel(
        cfg, cube, all_drought, all_drought_obs, glob.glob(input_filenames)
    )
