...
pyplot_kwargs: dict, optional
    ...
    to something like  ``ambiguous_project``. Examples: ``title: 'Awesome Plot
    of {long_name}'``, ``xlabel: '{short_name}'``, ``xlim: [0, 5]``.
standard_dev_kwargs: dict, optional
    Optional keyword arguments for plotting annual variability. 
...
class MultiDatasets(MonitorBase):
    ...
    def __init__(self, config):
        ...
        for (plot_type, plot_options) in self.plots.items():
            ...
            # Defaults for map, profile and levtime plots
            if plot_type in ('map', 'profile', 'levtime'):
            ...
            # Defaults for levtime plots    
            if plot_type == 'levtime' :
                self.plots[plot_type].setdefault('log_y', True)
    ...
    def _add_stats(self, plot_type, axes, dim_coords, dataset,
        ...
        # Mean, is implemented in the preprocessor functions now
    ...
    def _get_cbar_kwargs(self, plot_type, bias=False):
        ...
        elif plot_type in ('profile', 'levtime'):
    ...
    def _get_plot_kwargs(self, plot_type, dataset, bias=False):
        ...
        if plot_type in ('timeseries', 'annual_cycle', 'diurnal_cycle', '2d_profile'):
    ...
    def _plot_map_with_ref(self, plot_func, dataset, ref_dataset):
        ...
        with mpl.rc_context(self._get_custom_mpl_rc_params(plot_type)):
            ...
            cbar_bias.ax.tick_params(labelsize=5)
            self._add_stats(plot_type, axes_bias, dim_coords_dat, dataset,
                            ref_dataset)

            # Customize plot
            # Use timerange in title if present (for submonthly data)
            #if 'timerange' in dataset:
            #    fig.suptitle(f"{dataset['long_name']} ({dataset['timerange'][6:8]}/{dataset['timerange'][4:6]}/{dataset['timerange'][:4]} -"
            #                 f" {dataset['timerange'][-2:]}/{dataset['timerange'][-4:-2]}/{dataset['timerange'][-6:-2]})")
            # Use start_year to end_year in title for climatologies
            fig.suptitle(f"{dataset['long_name']}")
    ...
    def _plot_profile_without_ref(self, plot_func, dataset):
        ...
        with mpl.rc_context(self._get_custom_mpl_rc_params(plot_type)):
            ...
            if 'timerange' in dataset:
                fig.suptitle(f"{dataset['long_name']} ({dataset['timerange'][6:8]}/{dataset['timerange'][4:6]}/{dataset['timerange'][:4]} -"
                             f" {dataset['timerange'][-2:]}/{dataset['timerange'][-4:-2]}/{dataset['timerange'][-8:-4]})")
            # Use start_year to end_year in title for climatologies
            else:
                fig.suptitle(f"{dataset['long_name']} ({dataset['start_year']}-"
                         f"{dataset['end_year']})")
    ...
    def _plot_levtime_with_ref(self, plot_func, dataset, ref_dataset):
        """Plot levtime plot for single dataset with a reference dataset."""
        plot_type = 'levtime'
        logger.info("Plotting levtime with reference dataset '%s' for '%s'",
                    self._get_label(ref_dataset), self._get_label(dataset))
        ...
        with mpl.rc_context(self._get_custom_mpl_rc_params(plot_type)):
            ...
            axes_bias.set_ylabel('air pressure [hPa]')
            cbar_kwargs_bias = self._get_cbar_kwargs(plot_type, bias=True)
            cbar_bias = fig.colorbar(plot_bias, ax=axes_bias,
                                     **cbar_kwargs_bias)
            cbar_bias.set_label(
            ...
            if 'timerange' in dataset:
                fig.suptitle(f"{dataset['long_name']} ({dataset['timerange'][6:8]}/{dataset['timerange'][4:6]}/{dataset['timerange'][:4]} -"
                             f" {dataset['timerange'][-2:]}/{dataset['timerange'][-4:-2]}/{dataset['timerange'][-6:-4]})")
            # Use start_year to end_year in title for climatologies
            else:
                fig.suptitle(f"{dataset['long_name']} ({dataset['start_year']}-"
                         f"{dataset['end_year']})")
    ...
    def _plot_levtime_without_ref(self, plot_func, dataset):
        """Plot levtime plot for single dataset without a reference dataset."""
        plot_type = 'levtime'
        logger.info("Plotting levtime without reference dataset for '%s'",
                    self._get_label(dataset))
        ...
        with mpl.rc_context(self._get_custom_mpl_rc_params(plot_type)):
            ...
            plot_kwargs = self._get_plot_kwargs(plot_type, dataset)
            plot_kwargs['axes'] = axes
            plot_levtime = plot_func(cube, **plot_kwargs)
            ...
            # Setup colorbar
            fontsize = self.plots[plot_type]['fontsize']
            colorbar = fig.colorbar(plot_levtime, ax=axes,
                                    **self._get_cbar_kwargs(plot_type))
            colorbar.set_label(self._get_cbar_label(plot_type, dataset),
                               fontsize=fontsize)
            colorbar.ax.tick_params(labelsize=fontsize)
            ...
            # Use start_year to end_year in title for climatologies
                ...
                fig.suptitle(f"{dataset['long_name']} ({dataset['start_year']}-"
                         f"{dataset['end_year']})")
            monthyeardayFmt = mdates.DateFormatter('%Y-%m-%d')
            axes.xaxis.set_major_formatter(monthyeardayFmt)
            _ = plt.xticks(rotation=90)
            z_coord = cube.coord(axis='Z')
            axes.set_ylabel('air pressure [hPa]')
            if self.plots[plot_type]['log_y']:
                axes.set_yscale('log')
                axes.get_yaxis().set_major_formatter(
                    FormatStrFormatter('%.1f'))
    ...
    def _check_cube_dimensions(cube, plot_type):
        ...
        expected_dimensions_dict = {
            ...
            'profile': (['latitude', 'air_pressure'],
                        ['latitude', 'altitude']),
            'timeseries': (['time'],),
            'levtime' : (['time', 'air_pressure'],
                         ['time', 'altitude']),
            '2d_profile' : (['altitude'], ['air_pressure'])
        }
    ...
    def create_annual_cycle_plot(self, datasets, short_name):
        ...
        for dataset in datasets:
            ...
            plot_kwargs = self._get_plot_kwargs(plot_type, dataset)
            plot_kwargs['axes'] = axes
            # check if plotting variablity is desired
            # note: it does not work with preprocessor climate statistics yet
            # look below
            standard_dev_kwargs = self.plots[
            plot_type].get('standard_dev_kwargs', {})

            if standard_dev_kwargs is True:
                ...
                plt.plot(cube_mean.coord("month_number").points,cube_mean.data,
                         label=dataset[cfg['facet_used_for_labels']], **plot_kwargs)
                plt.fill_between(cube_mean.coord("month_number").points,
                                 cube_std_neg.data,cube_std_pos.data, alpha=0.4)
            else:
            # Plot annual cycle
                iris.plot.plot(cube, **plot_kwargs)
...
#            provenance_logger.log(netcdf_path, provenance_record)
    ...
    def create_diurnal_cycle_plot(self, datasets, short_name):
    # Only works with the following preprocessor settings
    # climate_statistics:
    #   operator: mean
    #   period: hourly
...
#            self._check_cube_dimensions(cube, plot_type)
            ...
            if standard_dev_kwargs is True:
                ...
                cube_std_neg= cube_mean-cube_std
                cube_std_pos= cube_mean+cube_std
                plt.plot(cube_mean.coord("hour").points,cube_mean.data,
                         label=dataset[cfg['facet_used_for_labels']], **plot_kwargs)
                plt.fill_between(cube_mean.coord("hour").points,
                                 cube_std_neg.data,cube_std_pos.data, alpha=0.4)
            else:
            # Plot diurnal cycle
                iris.plot.plot(cube, **plot_kwargs)
        ...
        axes.set_ylabel(f"{short_name} [{multi_dataset_facets['units']}]")
        #axes.set_xticks(range(1, 25), [str(m) for m in range(1, 25)])
...
#            provenance_logger.log(netcdf_path, provenance_record)
    ...
    def create_2d_profile_plot(self, datasets, short_name):
        """Create time series plot."""
        plot_type = '2d_profile'
        if plot_type not in self.plots:
            return
        ...
        # Default plot appearance
        multi_dataset_facets = self._get_multi_dataset_facets(datasets)
        axes.set_title( fig.suptitle(f"{dataset['long_name']} ({dataset['timerange'][6:8]}/{dataset['timerange'][4:6]}/{dataset['timerange'][:4]} -"
                             f" {dataset['timerange'][-2:]}/{dataset['timerange'][-4:-2]}/{dataset['timerange'][-8:-4]})"))
    ...
    def create_levtime_plot(self, datasets, short_name):
        """Create profile plot."""
        plot_type = 'levtime'
        if plot_type not in self.plots:
            return
        ...
        # Get reference dataset if possible
        ref_dataset = None #self._get_reference_dataset(datasets, short_name)
        if ref_dataset is None:
            logger.info("Plotting %s without reference dataset", plot_type)
        else:
            logger.info("Plotting %s with reference dataset '%s'", plot_type,
        ...
        for dataset in datasets:
            ...
            ancestors = [dataset['filename']]
            if ref_dataset is None:
                (plot_path, netcdf_paths) = ( self._plot_levtime_without_ref(plot_func,                                 dataset))
                caption = (
                    f"levtime of {dataset['long_name']} of dataset "
                    f"{dataset['dataset']} (project {dataset['project']}) "
                    f"from {dataset['start_year']} to {dataset['end_year']}."
                )
            else:
                (plot_path, netcdf_paths) = (
                    self._plot_levtime_with_ref(plot_func, dataset,
                                                ref_dataset)
                )
                caption = (
                    f"levtie of {dataset['long_name']} of dataset "
                    f"{dataset['dataset']} (project {dataset['project']}) "
                    f"including bias relative to {ref_dataset['dataset']} "
                    f"(project {ref_dataset['project']}) from "
                    f"{dataset['start_year']} to {dataset['end_year']}."
    ...
    def compute(self):
        ...
        for (short_name, datasets) in self.grouped_input_data.items():
            ...
            self.create_levtime_plot(datasets, short_name)
            self.create_map_plot(datasets, short_name)
            self.create_profile_plot(datasets, short_name)
            self.create_2d_profile_plot(datasets, short_name)
            self.create_diurnal_cycle_plot(datasets, short_name)
