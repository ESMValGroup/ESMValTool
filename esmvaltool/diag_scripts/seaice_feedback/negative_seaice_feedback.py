...
class NegativeSeaIceFeedback(object):
    ...
    def compute(self):
        ...
        for dataset in self.datasets:
            ...
            nf, stats, _ = self.negative_seaice_feedback(
    ...
    def compute_volume(self, avg_thick, cellarea, mask=1):
        ...
        if avg_thick.coords('time'):
            nt = avg_thick.shape[0]
            vol = [np.sum(avg_thick.data[jt, :, :] * cellarea.data * mask.data) / 1e12
                   for jt in range(nt)]
            vol = np.asarray(vol)
    ...
    def negative_seaice_feedback(self, volume, period, order=1):
        ...
        imin = [t + np.nanargmin(volume.data[t:t + period]) for t in range(0, volume.size, period)]

        # 2. Locate the maxima for each year
        imax = [t + np.nanargmax(volume.data[t:t + period]) for t in np.arange(0, volume.size, period)]

        # 3. Detrend series. A one-year shift is introduced to make sure we
        #    compute volume production *after* the summer minimum
        v_min = self.detrend(volume[imin[:-1]], order=order)
        dv = self.detrend(volume[imax[1:]] - volume[imin[:-1]], order=order)
        ...
        if np.max(v_min) == 0.0 or np.max(dv == 0.0):
            ...
            tstat = r / np.sqrt((1 - r ** 2) / (len(v_min) - 2))  # The t-statistic.
            # Under the null hypothesis of no correlation,
            # tstat follows a student's law with  N - 2 dof.
            pval = 1.0 - scipy.stats.t.cdf(np.abs(tstat), len(v_min) - 2)
