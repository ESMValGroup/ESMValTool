...
hyint.diagnostic <- function(work_dir, infile, model_idx, season, 
  ...
  # file opening
  pr_list <- ncdf.opener.universal(infile, namevar = "pr", 
  ...
  if (external_r95[1] == F) {
    ...
    if (external_r95[1] == "HIST") { # if required, use HyInt file from historical period
      external_r95 <- getfilename.indices(work_dir, diag_base, model_idx, 
                      season, hist = T, hist_years = norm_years)
    }
    r95_idx <- model_idx # assume each model has its r95_threshold file
    if (length(external_r95) == 1) {
      r95_idx <- 1
    } # if list of files with r95_threshold has only 1 entry, use that for all models
    print(paste(diag_base, ": loading external r95_threshold data from ", 
                external_r95[r95_idx]))
    r95_threshold <- ncdf.opener(external_r95[r95_idx], "r95_threshold", 
                     rotate = "no")
  }
  ...
  for (iyear in 1:nyear) {
    ...
    pr_year_r95 <- pr_year
    pr_year_r95[ret_below_r95] <- NA # actual precipitation but with NA on days with pr < reference 95% quantile
  ...
  if (external_norm[1] == F) { # calculate normalization function
    ...
    if (external_norm[1] == "HIST") { # if required, use HyInt file from historical period
      ...
      external_norm <- getfilename.indices(work_dir, diag_base, model_idx,
                       season, hist = T, hist_years = norm_years)
    }
    if (length(external_norm) == 1) {
      mean_idx <- 1
    } # if list of files with normalization functions has only 1 entry, use that for all models
    print(paste(diag_base, ": loading external normalization data from ", 
    ...
    pry_mean_sd <- ncdf.opener(external_norm[mean_idx], "pry_mean_sd", 
                   rotate = "no")
    dsl_mean_sd <- ncdf.opener(external_norm[mean_idx], "dsl_mean_sd", 
                   rotate = "no")
    wsl_mean_sd <- ncdf.opener(external_norm[mean_idx], "wsl_mean_sd", 
                   rotate = "no")
    int_mean_sd <- ncdf.opener(external_norm[mean_idx], "int_mean_sd", 
                   rotate = "no")
    pa_mean_sd <- ncdf.opener(external_norm[mean_idx], "pa_mean_sd", 
                  rotate = "no")
    r95_mean_sd <- ncdf.opener(external_norm[mean_idx], "r95_mean_sd", 
                   rotate = "no")
  }
  ...
  if (external_norm[1] == F) { # calculate or load hyint_mean from file to be consistent with other indices
    ...
    hyint_mean <- ncdf.opener(external_norm[mean_idx], "hyint_mean", 
                  rotate = "no")
    hyint_mean_sd <- ncdf.opener(external_norm[mean_idx], "hyint_mean_sd", 
                     rotate = "no")
  }
  ...
  t <- ncdim_def(timedimname, "years", years, unlim = T, calendar = tcal, 
  ...
  for (var in field_list) {
    ...
    var_ncdf <- ncvar_def(var, unit, list(x, y, t), -999, longname = longvar, 
    ...
    if ((var == "pry_mean") | (var == "int_mean") | (var == "dsl_mean") | 
        ...
        (var == "r95_threshold")) {
      var_ncdf <- ncvar_def(var, unit, list(x, y), -999, longname = longvar, 
                            prec = "single", compression = 1)
    }
  ...
  for (var in field_list) {
    ...
    ncvar_put(ncfile, var, get(paste0("field", var)), start = rep(1, ndims), 
              count = rep(-1, ndims))
  }
  nc_close(ncfile)
