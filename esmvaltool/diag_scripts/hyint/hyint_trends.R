...
hyint.trends <- function(work_dir, model_idx, season) {
  ...
  if (!all(is.element(field_names, names(nc$var)))) {
    ...
    print(paste("HyInt_trends: missing variable in input indices file: ", 
                 missing))
    nc_close(nc)
  ...
  if (maskSeaLand) {
    ...
    if (!file.exists(topo_file_idx)) {
      createLandSeaMask(regrid = gridfile, ref_file = infile, 
  ...
  for (var in field_names) {
    ...
    if (maskSeaLand) {
      rfield <- apply.elevation.mask(rfield, relevation, seaLandElevation, 
    ...
    for (ireg in 1:nregions) {
      ...
      tfield[ireg, ] <- calc.region.timeseries(ics, ipsilon, rfield, 
                        regions[iselreg, ], weighted_mean = weight_tseries)
      tfield_sd[ireg, ] <- calc.region.timeseries(ics, ipsilon, rfield, 
                           regions[iselreg, ], calc_sd = T)
    }
  ...
  varregions <- ncvar_def(
    ...
    "region boundaries", prec = "single", compression = 1
  )
  ...
  for (var in namelist) {
    ...
    ncvar_put(ncfile, var, tmp.field, start = rep(1, ndims), 
              count = rep(-1, ndims))
