...
#-------------P. Davini (Oct 2014)-------------------#
######################################################
miles_block_fast <- function(dataset, expid, ens, year1, year2, season,
                             z500filename, FILESDIR, doforce) {
  t0 <- proc.time()
  ...
  savefile1 <- file_builder(
    FILESDIR, "Block", "BlockClim", dataset,
    expid, ens, year1, year2, season
  )
  savefile2 <- file_builder(
    FILESDIR, "Block", "BlockFull", dataset,
    expid, ens, year1, year2, season
  )
  ...
  fieldlist <- ncdf_opener_universal(nomefile,
    namevar = "zg",
    tmonths = timeseason, tyears = years,
    rotate = "full"
  )
  print(str(fieldlist))
  ...
  cn <- apply(totrwb, c(1, 2), function(x) sum(x[x == (-10)], na.rm = T)) /
    (totdays) * (-10)
  acn <- apply(totrwb, c(1, 2), function(x) sum(x[x == (10)], na.rm = T)) /
    (totdays) * (10)
  ...
  block <- blocking_persistence(large, minduration = 5, time.array = etime)
  ...
  fieldlist <- c(
    "TM90", "InstBlock", "ExtraBlock", "Z500", "MGI", "BI",
    "CN", "ACN", "BlockEvents", "LongBlockEvents",
    "DurationEvents", "NumberEvents"
  )
  full_fieldlist <- c(
    "TM90", "InstBlock", "ExtraBlock", "Z500", "MGI", "BI",
    "CN", "ACN", "BlockEvents", "LongBlockEvents"
  ...
  t1 <- ncdim_def("time", TIME, 0,
    unlim = T, calendar = tcal,
    longname = "time"
  )
  t2 <- ncdim_def("time", TIME, fulltime,
    unlim = T, calendar = tcal,
    longname = "time"
  )
  ...
  for (var in fieldlist) {
    ...
    if (var == "TM90") {
      var_ncdf <- ncvar_def(var, unit, list(x, t = t1), -999,
        longname = longvar, prec = "single", compression = 1
      )
      full_var_ncdf <- ncvar_def(var, unit, list(x, t = t2), -999,
        longname = longvar, prec = "single", compression = 1
      )
    } else {
      var_ncdf <- ncvar_def(var, unit, list(x, y, z, t = t1), -999,
        longname = longvar, prec = "single", compression = 1
      )
      full_var_ncdf <- ncvar_def(var, unit, list(x, y, z, t = t2), -999,
        longname = longvar, prec = "single", compression = 1
      )
    }
  ...
  for (var in fieldlist) {
    ...
    ncvar_put(ncfile1, var, get(paste0("field", var)),
      start = rep(1, ndims),
      count = rep(-1, ndims)
    )
  }
  ...
  for (var in full_fieldlist) {
    ...
    ncvar_put(ncfile2, var, get(paste0("full_field", var)),
      start = rep(1, ndims), count = rep(-1, ndims)
    )
  }
  nc_close(ncfile2)
