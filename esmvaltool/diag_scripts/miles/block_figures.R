...
miles_block_figures <- function(dataset, expid, ens, year1, year2,
  ...
  fieldlist <- c(
    "InstBlock", "ExtraBlock", "Z500", "MGI", "BI", "CN", "ACN",
    "BlockEvents", "LongBlockEvents", "DurationEvents",
    "NumberEvents", "TM90"
  )
  ...
  for (field in fieldlist) {
    ...
    nomefile <- file_builder(
      FILESDIR, "Block", "BlockClim", dataset, expid,
      ens, year1, year2, season
    )
  ...
  for (field in fieldlist) {
    ...
    if (REFDIR != FILESDIR) {
      nomefile_ref <- paste0(
        file.path(REFDIR, "Block"), "/BlockClim_",   # nolint
      ...
      nomefile_ref <- file_builder(
        FILESDIR, "Block", "BlockClim",
        dataset_ref, expid_ref, ens_ref, year1_ref, year2_ref, season
      )
    }

    field_ref <- ncdf_opener(nomefile_ref, namevar = field, rotate = "no")
  ...
  for (field in fieldlist) {
    ...
    figname <- fig_builder(
      FIGDIR, "Block", field, dataset, expid, ens, year1,
    ...
    if (field == "TM90") {
      ...
      # panels option
      par(
        cex.main = 2, cex.axis = 1.5, cex.lab = 1.5, mar = c(5, 5, 4, 3),
        oma = c(0, 0, 0, 0)
      )
      ...
      plot(ics2, field_exp2,
        type = "l", lwd = lwdline, ylim = fp$lev_field,
        main = fp$title_name, xlab = "Longitude", ylab = fp$legend_unit,
        col = tm90cols[1]
      )
      points(ics2, field_ref2,
        type = "l", lwd = lwdline, lty = 1,
        col = tm90cols[2]
      )
      grid()
      legend(100, 30,
        legend = c(info_exp, info_ref), lwd = lwdline,
        lty = c(1, 1), col = tm90cols, bg = "white", cex = 1.
      )
    ...
    im <- plot_prepare(ics, ipsilon, field_exp,
      proj = map_projection,
      lat_lim = lat_lim
    )
    filled_contour3(im$x, im$y, im$z,
      xlab = im$xlab, ylab = im$ylab,
      main = paste(info_exp), levels = fp$lev_field,
      color.palette = fp$color_field, xlim = im$xlim,
      ylim = im$ylim, axes = im$axes
    )
    mtext(fp$title_name, side = 3, line = .5, outer = TRUE, cex = 2, font = 2)
    proj_addland(proj = map_projection)

    # reference field plot
    im <- plot_prepare(ics, ipsilon, field_ref,
      proj = map_projection, lat_lim = lat_lim
    )
    filled_contour3(im$x, im$y, im$z,
      xlab = im$xlab, ylab = im$ylab,
      main = paste(info_ref), levels = fp$lev_field,
      color.palette = fp$color_field, xlim = im$xlim,
      ylim = im$ylim, axes = im$axes
    )
    proj_addland(proj = map_projection)
    image_scale3(volcano,
      ...
      colorbar.width = 1 * af, line.label = fp$legend_distance
    )

    # delta field plot
    im <- plot_prepare(ics, ipsilon, field_exp - field_ref,
                       proj = map_projection, lat_lim = lat_lim)
    filled_contour3(im$x, im$y, im$z,
      xlab = im$xlab, ylab = im$ylab,
      main = paste("Difference"), levels = fp$lev_diff,
      color.palette = fp$color_diff, xlim = im$xlim,
      ylim = im$ylim, axes = im$axes
    )
    proj_addland(proj = map_projection)
    image_scale3(volcano,
