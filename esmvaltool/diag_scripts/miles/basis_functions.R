...
sector_details <- function(SECTOR) {
  ...
  out <- list(
    lons = lons, lonssel = lonssel, lats = lats, latssel = latssel,
    name = namesec
  )
  return(out)
}
...
# info string creator
info_builder <- function(dataset, expid, ens, year1, year2, season) {
...
file_builder <- function(DATADIR, dir_name, file_name, dataset, expid, ens,
                         year1, year2, season) {
...
fig_builder <- function(FIGDIR, dir_name, file_name, dataset, expid,
...
ncdf_opener_universal <- function(namefile, namevar = NULL, namelon = NULL,
  ...
  if (is.null(namevar)) {
    ...
    if (length(namevar) > 1) {
      print(namevar)
      stop(paste("More than one var in the files, please select it with"),
        " namevar=yourvar",
        sep = ""
      )
    }
...
ncdf_opener <- function(namefile, namevar = NULL, namelon = NULL,
  ...
  field <- ncdf_opener_universal(
    namefile, namevar, namelon, namelat,
    tmonths, tyears, rotate, interp2grid,
    grid, remap_method, exportlonlat, verbose
  )
  return(field$field)
}
...
filled_contour3 <-
  function(x = seq(0, 1, length.out = nrow(z)),
             ...
             col = color.palette(length(levels) - 1), extend = TRUE,
...
image_scale3 <- function(z, levels, color.palette = heat.colors,
  ...
  new.fig <- c(
    ...
    old.fig[2] - 0.03 * xscal - lp, old.fig[3] + 0.1 * yscal,
    old.fig[4] - 0.1 * yscal
  )
  ...
  plot(1, 1,
    t = "n", ylim = ylim, xlim = xlim, axes = FALSE,
...
proj_plot <- function(lon, lat, field, lmin = NULL, proj = "azequalarea",
                      param = NULL, orient = c(90, 0, 0), npoints = 201) {
  ...
  # project grid
  proj.grid <- mapproject(lon.grid, lat.grid,
    projection = proj,
    parameters = param, orientation = orient
  )

  # provide limits for future plots (for polar projection)
  limiter <- mapproject(c(0, 90, 180, 270), rep(lmin, 4),
  ...
  # plot grid
  lon.plot <- seq(min(proj.grid$x, na.rm = T), max(proj.grid$x, na.rm = T),
    length.out = npoints
  )
  lat.plot <- seq(min(proj.grid$y, na.rm = T), max(proj.grid$y, na.rm = T),
    length.out = npoints
  )

  # interpolation (akima needed)
  good <- is.finite(field) & is.finite(proj.grid$x) & is.finite(proj.grid$y)
  projected <- interp(proj.grid$x[good], proj.grid$y[good], field[good],
    lon.plot, lat.plot,
    duplicate = "strip"
  )
  return(projected = list(
    x = projected$x, y = projected$y,
    z = projected$z, xlim = xlims, ylim = ylims
  ))
}

# addland function based on map which can handle projections
proj_addland <- function(proj = "no", orient = c(90, 0, 0),
                         param = NULL, color = "black") {
  ...
  if (proj == "no") {
    ...
    box()
    map("world",
      add = T, projection = proj, orientation = orient,
      parameter = param, interior = F, exact = F, boundary = T
    )
...
plot_prepare <- function(ics, ipsilon, field, proj, lat_lim) {
  if (proj == "no") {
    outfile <- list(
      x = ics, y = ipsilon, z = field, xlim = range(ics),
      ylim = lat_lim, xlab = "Longitude", ylab = "Latitude", axes = T
    )
  } else {
    field[is.na(field)] <- 0
    p <- proj_plot(ics, ipsilon, field,
      lmin = lat_lim[1], proj = proj,
      param = NULL, orient = c(90, 0, 0), npoints = 80
    )
    outfile <- list(
      x = p$x, y = p$y, z = p$z, xlim = p$xlim,
      ylim = p$ylim, xlab = "", ylab = "", axes = F
    )
  }
  return(outfile)
}
...
field_details <- function(field) {
  ...
  out <- list(
    color_field = color_field, color_diff = color_diff,
    lev_field = lev_field, lev_diff = lev_diff, lev_hist = lev_hist,
    legend_unit = legend_unit, legend_distance = legend_distance,
...
blocking_persistence <- function(field, minduration = 5, time.array) {
  ...
  newfield <- apply(field, c(1, 2), function(x) pers2(x,
  ...
  maxdim <- max(apply(
    newfield, c(1, 2),
    function(x) length(rle(x)$length[which(rle(x)$values == 1)])
  ))
  events <- apply(
    newfield, c(1, 2),
    function(x) c(
        rle(x)$lengths[which(rle(x)$values == 1)],
        rep(NA, maxdim - length(rle(x)$length[which(rle(x)$values == 1)]))
      )
  )
  ...
  out <- list(
    track = newfield, percentage = meanfield, duration = duration,
    events = events, nevents = nevents
  )
...
largescale_extension_if <- function(ics, ipsilon, field) {
  ...
  time <- which(apply(field, 3, max) != 0) # elements length of the dataset
  # (removing not blocked days)

  print(paste(
    "Box dimension:", passo * 2 * xreso, "° lon x ",
    vertical * 2 * yreso, "° lat"
  ))
...
eofs <- function(lon, lat, field, neof = 4, xlim, ylim, method = "SVD",
                 do_standardize = F, do_regression = F) {
  ...
  out <- list(
    pattern = pattern, coeff = coefficient,
    variance = variance, regression = regression
  )
  return(out)
}

eofs_coeff <- function(lon, lat, field, eof_object, do_standardize = F) {
...
regimes <- function(lon, lat, field, ncluster = 4, ntime = 1000,
                    neof = 10, xlim, ylim, alg = "Hartigan-Wong") {
  ...
  print("Finalize...")
  out <- list(
    cluster = cluster, frequencies = frequencies[kk],
    regimes = compose[, , kk], tot.withinss = regimes$tot.withinss
  )
  return(out)
}
...
regimes2 <- function(lon, lat, field, ncluster = 4, ntime = 1000, minvar = 0.8,
  ...
  print(paste(
    "Retaining", reqpc,
    "PCs to fullfil minimum explained variance required (", minvar * 100, "%)"
  ))
