...
info_output(paste0("plot - ", diag_script, " (var: ", variables[1], ")"),
            verbosity, 1)
info_output("+++++++++++++++++++++++++++++++++++++++++++++++++", verbosity, 1)
...
for (model_idx in c(1:(length(models_name)))) {
  ...
  system2("diag_scripts/aux/miles/z500_prepare.sh", c(exp, toString(year1),
                                      toString(year2), infile, zdirfile))
  for (seas in seasons) {
    miles_regimes_fast(year1 = year1, year2 = year2, exp = exp, season = seas,
        z500filename = zdirfile, FILESDIR = work_dir, nclusters = nclusters)
  }
}
