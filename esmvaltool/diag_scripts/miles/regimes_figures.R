...
miles_regimes_figures <- function(dataset, expid, ens, year1, year2,
                                  dataset_ref, expid_ref, ens_ref,
                                  year1_ref, year2_ref, season,
                                  FIGDIR, FILESDIR, REFDIR, nclusters) {
  ...
  nomefile <- file_builder(FILESDIR, "Regimes", "RegimesPattern", dataset,
                           expid, ens, year1, year2, season)
  frequencies_exp <- ncdf_opener(nomefile, "Frequencies")
  regimes_exp <- ncdf_opener(nomefile, namevar = "Regimes", rotate = "no")
  ...
  if (REFDIR != FILESDIR) {
    nomefile_ref <- paste0(file.path(REFDIR, "Regimes"), "/RegimesPattern_",
                           dataset_ref, "_", year1_ref, "_", year2_ref,
                           "_", season, ".nc")
  } else {
    # use file.builder to create the path of the blocking files
    nomefile_ref <- file_builder(FILESDIR, "Regimes", "RegimesPattern",
                                 dataset_ref, expid_ref, ens_ref,
                                 year1_ref, year2_ref, season)
  }

  frequencies_ref <- ncdf_opener(nomefile_ref, "Frequencies")
  regimes_ref <- ncdf_opener(nomefile_ref, namevar = "Regimes", rotate = "no")
  ...
  for (name in names_ref) {
    ...
    figname <- fig_builder(FIGDIR, "Regimes", paste0("Regime", ii),
    ...
    filled_contour3(im$x, im$y, im$z, xlab = im$xlab, ylab = im$ylab,
                    ...
                    ylim = im$ylim, axes = im$axes)
    proj_addland(proj = map_projection)
    text(varpoints[1], varpoints[2], paste("Frequencies: ",
         round(frequencies_ref[ii], 2), "%", sep = ""), cex = 2)
    image_scale3(volcano, levels = lev_field, color.palette = palette3,
                 colorbar.label = "m", cex.colorbar = imgscl_colorbar,
                 cex.label = imgscl_label, colorbar.width = 1 * af,
                 line.label = imgscl_line)

    # delta field plot
    im <- plot_prepare(ics, ipsilon, regimes_exp[, , ii] - regimes_ref[, , jj],
                       proj = map_projection, lat_lim = lat_lim)
    filled_contour3(im$x, im$y, im$z, xlab = im$xlab, ylab = im$ylab,
                    main = paste("Difference"), levels = lev_diff,
                    color.palette = palette2, xlim = im$xlim,
                    ylim = im$ylim, axes = im$axes)
    proj_addland(proj = map_projection)
    image_scale3(volcano, levels = lev_diff, color.palette = palette2,
                 colorbar.label = "m", cex.colorbar = imgscl_colorbar,
                 cex.label = imgscl_label, colorbar.width = 1 * af,
                 line.label = imgscl_line)
