...
miles_regimes_fast <- function(dataset, expid, ens, year1, year2, season,
                               z500filename, FILESDIR, nclusters, doforce) {
  ...
  savefile1 <- file_builder(FILESDIR, "Regimes", "RegimesPattern", dataset,
  ...
  # new file opening
  fieldlist <- ncdf_opener_universal(z500filename, namevar = "zg",
  ...
  # variance evaluation
  weather_regimes <- regimes2(ics, ipsilon, Z500anom, ncluster = nclusters,
                              ntime = 1000, minvar = 0.8, xlim, ylim,
                              alg = "Hartigan-Wong")
  ...
  for (i in 1:nclusters) {
    ...
    if (min(distmm) < min_dist_in_deg) {
      ...
      if (i > 1 & any(names[i] == names[1:max(c(1, i - 1))])) {
        print(paste("Warning: double assignation of the same regime.",
  ...
  t <- ncdim_def("time", TIME, fulltime, calendar = tcal,
                 longname = "time", unlim = T)
  ...
  pattern_ncdf <- ncvar_def("Regimes", unit, list(x, y, cl), -999,
                            longname = longvar, prec = "single",
                            compression = 1)

  unit <- paste0("0-", nclusters)
  longvar <- "Weather Regimes Cluster Index"
  cluster_ncdf <- ncvar_def("Indices", unit, list(t), -999, longname = longvar,
                            prec = "single", compression = 1)

  unit <- "%"
  longvar <- "Weather Regimes Frequencies"
  frequencies_ncdf <- ncvar_def("Frequencies", unit, list(cl), -999,
                                longname = longvar, prec = "single",
                                compression = 1)
  ...
  # saving file
  ncfile1 <- nc_create(savefile1, list(pattern_ncdf, cluster_ncdf,
                                       frequencies_ncdf, names_ncdf))
  ncvar_put(ncfile1, "Regimes", weather_regimes$regimes, start = c(1, 1, 1),
            count = c(-1, -1, -1))
  ncvar_put(ncfile1, "Indices", weather_regimes$cluster, start = c(1),
            count = c(-1))
  ncvar_put(ncfile1, "Frequencies", weather_regimes$frequencies,
            start = c(1), count = c(-1))
