...
#-------------P. Davini (Feb 2018)-------------------#
######################################################
miles_eofs_fast <- function(dataset, expid, ens, year1, year2, season,
  ...
  fieldlist <- ncdf_opener_universal(nomefile, "zg",
    tmonths = timeseason,
    tyears = years, rotate = rotation
  )
  print(str(fieldlist))
  ...
  t <- ncdim_def("time", TIME, monthtime,
    calendar = tcal,
  ...
  pattern_ncdf <- ncvar_def("Patterns", unit, list(x, y, z, ef), -999,
    longname = longvar, prec = "single", compression = 1
  )

  unit <- "m"
  longvar <- "EOFs Linear Regressions"
  regression_ncdf <- ncvar_def("Regressions", unit, list(x, y, z, ef), -999,
    longname = longvar, prec = "single", compression = 1
  )

  unit <- paste0("0-", neofs)
  longvar <- "PCs timeseries"
  pc_ncdf <- ncvar_def("PCs", unit, list(ef, t), -999,
    longname = longvar, prec = "single", compression = 1
  )

  unit <- "%"
  longvar <- "EOFs variance"
  variance_ncdf <- ncvar_def("Variances", unit, list(ef), -999,
    longname = longvar, prec = "single", compression = 1
  )
  ...
  ncvar_put(ncfile1, "Patterns", expanded_pattern,
    start = c(1, 1, 1, 1), count = c(-1, -1, -1, -1)
  )
  ncvar_put(ncfile1, "Regressions", EOFS$regression,
    start = c(1, 1, 1, 1), count = c(-1, -1, -1, -1)
  )
