"""
Personal diagnostics for Arctic-midlatitude
research.
...
from esmvalcore.preprocessor import area_statistics, meridional_statistics, zonal_statistics, anomalies
...
#        print (diagnostic_file)
        ...
        if key == "ERA5":
            cube_list = iris.cube.CubeList([pv, tas, tas_baffin, tas_sib, psl_Ural, psl_Sib, psl_Aleut, zon_wind, hf])
            iris.save(cube_list, diagnostic_file)
        elif key == "HadISST":
            cube_list = iris.cube.CubeList([sic_BK, sic_Ok])
            iris.save(cube_list, diagnostic_file)
        else:
            cube_list = iris.cube.CubeList([pv, tas, tas_baffin, tas_sib, psl_Ural, psl_Sib, psl_Aleut, zon_wind, hf, sic_BK, sic_Ok])
            iris.save(cube_list, diagnostic_file)
