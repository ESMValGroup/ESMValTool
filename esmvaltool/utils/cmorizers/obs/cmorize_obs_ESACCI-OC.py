...
from .utilities import (_set_global_atts, _fix_coords, _fix_var_metadata,
                        _read_cmor_config, _save_variable,
                        constant_metadata)
...
def _fix_attr_cmip5(out_dir, var):
    ...
    datt={ 'standard_name':'depth', 'long_name':'depth',
        'units':'m' , 'axis':'Z' , 'positive':'down', '_FillValue': False }
...
def da_coarsen(da, bin):
    ...
    newda.values = data.reshape(
        [dd[0],dd[1]//bin,bin,dd[2]//bin,bin]
        ).mean(-1).mean(-2)
...
def merge_data(in_dir, out_dir, raw_info, bin):
    ...
    for ff in in_files:
        ...
        for delkey in ['grid_mapping','ancillary_variables',
                'parameter_vocab_uri'] :
            del da.attrs[delkey]
        ...
        if ff == in_files[0]:
            ...
            if (do_bin) :
                comment = str(bin)+' by '+str(bin) +' cells average binning' 
...
def cmorization(in_dir, out_dir):
    ...
    for var, vals in CFG['variables'].items():
        ...
        inpfile, addinfo = merge_data(in_dir, out_dir, raw_info, CFG['custom']['bin_size'] )
