...
def list_all_files(file_dict, CMIP,debug = True):
    ...
    for key, value in file_dict.items():
        print('list_all_files', key, value)
...
def file_to_recipe_dataset(fn, CMIP, file_dict, debug = True):
    """
    ...
    for base_key, fn_key in zip(basefile_split, fnfile_split):
        ...
        if base_key == '*.nc':
            ...
            print(base_key, fn_key,':', start_year,end_year)
            assert 0
...
def condense_times(files_recipe_dicts):
    ...
    groups = {}
    print('condense_times', files_recipe_dicts)
    ...
    for fn, output_dataset in files_recipe_dicts.items():
        ...
        #if 'start_year' in tmp_dict:
        start_year = tmp_dict.pop('start_year')
        #if 'end_year' in tmp_dict:
...
#           if 'short_name' not in new_dict:
#               new_dict['{short_name}'] = var_dict[variable]

            print('parse_recipe_to_dicts', variable, '\n',new_dict,'\n', var_dict)

            # for each key in the variable:
            for var_key, var_value in var_dict.items():
                print('var_key', var_key, 'var_value', var_value)
                var_key = add_brackets(var_key)
                print('    new var_key', var_key)
                if var_key in new_dict:
                    print('diag', diag, 'variable', variable, 'var_key', var_key, 'in new dict', (var_key))
