...
# test body
##################
ncfile = '/badc/cmip5/data/cmip5/output1/NOAA-GFDL/GFDL-ESM2G/historical/mon/atmos/Amon/r1i1p1/latest/ta/ta_Amon_GFDL-ESM2G_historical_r1i1p1_200101-200512.nc'
ncdata = iris.load(ncfile)[3]
levels = [100000, 97500, 95000, 92500, 90000, 87500, 85000, 82500]
selected_lev_data = vinterp(ncdata, levels, 'linear')
iris.save(selected_lev_data, 'extract_levels_GFDL.nc')
c= iris.load_cube('extract_levels_GFDL.nc')
cdat = c.data
print('------------------------------------------------')
print('Cube data:')
print('------------------------------------------------')
print('Non interpolated level (existing) max value: ', np.ma.max(cdat[:, 0, :, :]))
print('Interpolated level max value: ', np.ma.max(cdat[:, 1, :, :])) 
