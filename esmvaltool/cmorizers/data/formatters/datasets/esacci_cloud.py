    ...
    while loop_date <= datetime(year1, 12, 1):
        if loop_date not in cube.coord("time").cells():
            ...
            nan_cube = _create_nan_cube(
                cubes[0], loop_date.year, loop_date.month, loop_date.day
            )
...
def _extract_variable_daily(
    ...
    for year in range(start_date.year, end_date.year + 1):
        ...
        for month in range(1, 13):
            ...
            for iday in range(1, num_days + 1):
                ...
                if filelist:
                    for inum, ifile in enumerate(filelist):
                        ...
                        try:
                            ...
                            for ivar, raw_name in enumerate(raw_var):
                                logger.info(
                                    "Extracting raw variable %s", raw_name
                                )
                                ...
                                if short_name in ["clt", "ctp"]:
                                    logger.info(
                                        "Append cube with variable %s",
                                        short_name,
                                    )
                                    cubes.append(daily_cube)

                                logger.info(
                                    "Append daylight cube with variable %s",
                                    short_name,
                                )
                    ...
                    logger.info(
                        "Fill missing day %s in month %s and year %s",
                        iday,
                        month,
                        year,
                    )
        ...
        cube_day = regrid(
            cube_day, target_grid="0.5x0.5", scheme="area_weighted"
        )
...
def _extract_variable_monthly(
    ...
    for year in range(start_date.year, end_date.year + 1):
        for month in range(1, 13):  # Loop through all months (1-12)
            ...
            for ifile in filelist:
                logger.info(
                    "CMORizing file %s for variable %s", ifile, short_name
                )
                ...
                try:
                    ...
                    if short_name == "clwvi":
                        ...
                        monthly_cube.data = (
                            monthly_cube.core_data() + cube_lwp.core_data()
                        )
                    ...
                    if any(
                        ...
                        logger.error(
                            "The file %s is not assigned to AM or PM", ifile
                        )
    ...
    if cubes_am:
        ...
        cube_am = regrid(
            cube_am, target_grid="0.5x0.5", scheme="area_weighted"
        )
        ...
        utils.save_variable(
            cube_am, short_name, out_dir, attrs, unlimited_dimensions=["time"]
        )
    ...
    if cubes_pm:
        ...
        # Regrid the cube to the target grid (e.g., 0.5x0.5)
        cube_pm = regrid(
            cube_pm, target_grid="0.5x0.5", scheme="area_weighted"
        )
        ...
        # Save the processed variable
        utils.save_variable(
            cube_pm, short_name, out_dir, attrs, unlimited_dimensions=["time"]
        )
    ...
    if cube_am and cube_pm:
        ...
        cube_total.data = np.mean(
            np.stack([cube_am.core_data(), cube_pm.core_data()]), axis=0
        )

        # Regrid the cube to the target grid (e.g., 0.5x0.5)
        cube_total = regrid(
            cube_total, target_grid="0.5x0.5", scheme="area_weighted"
        )

        # Fix units and handle any special cases like 'clt'
        if short_name == "clt":
            cube_total.data = (
                100 * cube_total.core_data()
            )  # Example conversion
        else:
            if "raw_units" in var:
