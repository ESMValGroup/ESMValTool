    ...
    for var, vals in cfg["variables"].items():
        ...
        for ir_mw in [file1, file2]:
            ...
            while current_date <= end_date:
                ...
                for _day in range(
                    1,
                    calendar.monthrange(current_date.year, current_date.month)[
                        1
                    ]
                    + 1,
                ):
                    ...
                    file_pattern_1 = (
                        f"{in_dir}/{vals['file_base']}-{vals['platform']}-"
                    ...
                    for filename in [
                        ...
                        try:
                            cube = iris.load_cube(
                                filename, vals["raw"], callback=load_callback
                            )
                        except OSError:  # chek what the exception is
                            logger.info(f"{current_date=}  MISSING")
                            if len(month_cube_list) > 0:
                                cube = _create_nan_cube(
                                    month_cube_list[0],
                                    current_date.year,
                                    current_date.month,
                                    current_date.day,
                ...
                # cmor_info = cfg["cmor_table"].get_variable(
                #    vals["mip"], vals["raw"]
                # )
