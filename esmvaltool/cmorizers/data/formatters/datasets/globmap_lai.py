...
def load_and_make_cubes(in_dir, filepath, year, variable):
    ...
    for filename in filelist:
        ...
        lon_points = np.array([hdf_data.attributes()['PROJ_UL_XY'][0] +
                               i * hdf_data.attributes()['PIXEL_SIZE']
                               for i in range(data_array.shape[1])]
                              )
        lat_points = np.array([hdf_data.attributes()['PROJ_UL_XY'][1] -
                               i * hdf_data.attributes()['PIXEL_SIZE']
        ...
        # use days since Jan 1st 1950
        # this fits CMOR fixing
