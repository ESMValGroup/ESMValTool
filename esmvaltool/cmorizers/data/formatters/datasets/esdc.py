...
def cmorization(in_dir, out_dir, cfg, cfg_user, start_date, end_date):
    ...
    def cmorize_cube(path):
        ...
        try:
            ...
            logger.info(f'Could not open zarr dataset "{path}": "KeyError: {e}"')
            return

        for short_name, var in variables.items():
            all_attributes = {**attributes, **var} # add the mip to the other attributes
            raw_name = var['raw']
            cube_xr = ds[raw_name]
    ...
    logger.debug(f'Pattern {Path(in_dir, version, filename_pattern)} matched: {matches}')
    
    if len(matches) != 0:
        for match in matches:
            cmorize_cube(match)
    else:
        logger.info("Dataset not found locally, attempting connection to the cloud.")
        if '*'in filename_pattern:
            logger.warning(f"For cloud connection, \"{filename_pattern}\" shouldn't contain wildcards")
