...
class DerivedVariable(DerivedVariableBase):
    ...
    # Required variables
    _required_variables = {
        'vars': [{'short_name': 'thetao', 'field': 'TO3M'}],
        'fx_files': ['volcello']
    }
    ...
    def calculate(self, cubes,):
        ...
        cube = cubes.extract_strict(Constraint(cube_func=lambda c: 
            c.var_name == 'thetao'))
        volume = cubes.extract_strict(Constraint(cube_func=lambda c: 
            c.var_name == 'volcello'))
        # 2. multiply with each other and with cprho0
        # try working with cubes here to save memory
        # this might not be possible because of non-broadcasting in Iris
#        cube *= volume * RHO_CP
